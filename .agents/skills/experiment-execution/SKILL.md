...
Comply with `.agents/rules/docs.md` and `.agents/rules/backend/pipeline.md`.
...
Comply with `.agents/rules/backend/pipeline.md`:
...
Kaggle submission is out of scope (per `.agents/rules/command.md`; explicit approval + `dev/submit`).
...
- **All user-facing output, a concise user question labels/descriptions, and result reports must be in Japanese** (per `.codex/AGENTS.md`)
