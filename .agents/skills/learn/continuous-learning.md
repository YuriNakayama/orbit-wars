...
```json
{
  "hooks": {
    "PreToolUse": [{
      ...
      "hooks": [{
        "type": "command",
        "command": ".agents/skills/learn/hooks/observe.sh pre"
      }]
    }],
    "PostToolUse": [{
      "matcher": "*",
      "hooks": [{
        "type": "command",
        "command": ".agents/skills/learn/hooks/observe.sh post"
      }]
    }]
  }
}
...
.codex/agents/start-observer.sh
```
...
- Existing `~/.agents/skills/learned/` skills still work
