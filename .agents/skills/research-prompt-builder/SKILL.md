...
In `--auto` mode, the remaining text in `the user request` (after removing `--auto`) is used as the research theme. For example: `/research-prompt-builder --auto 因果推論 heterogeneous treatment effect` → theme is "因果推論 heterogeneous treatment effect".
...
2. Gather missing information through step-by-step a concise user question hearings
3. Generate the structured prompt, display it in the conversation, and save to file

All hearings use a concise user question in selection format. Free-text input is accepted via the "Other" option (auto-appended). Always use the a concise user question tool rather than asking questions as plain text.

## Step 1: Understand the Research Theme

> **`--auto` mode**: Skip the initial hearing. Use the theme from `the user request` directly and infer the research field automatically.
...
- All hearings use a concise user question in selection format. Do not use text-based questions
...
- A single a concise user question can ask 1-4 questions simultaneously. Group related questions for efficient hearings
