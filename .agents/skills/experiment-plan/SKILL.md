...
3. **Free-text natural language** — heuristically infer family / case / hypothesis from the overview. If ambiguous, run **at most 1** `a concise user question`.
...
- If a plain `plan.md` (no `iter` prefix) exists and you are introducing the iter scheme for the first time, rename it to `iter1_plan.md` first, then write `iter2_plan.md`. Surface the rename to the user in one line before executing it (per `.agents/rules/docs.md`).
...
`a concise user question`, **at most 3 questions in one round**. Selection + Other format. Items already pinned by the skip list / `hypotheses.md` are **not** asked again.
...
- **Don't perform iteration migration silently.** When renaming an existing `plan.md` to `iter1_plan.md`, surface it in one line first (per `.agents/rules/docs.md`).
...
- **All user-facing output, a concise user question labels/descriptions, and the written plan body must be in Japanese** (per `.codex/AGENTS.md`)
