# Codex Tools — Allow-list Reference
...
| `web fetch` | Fetch a single URL | Medium (network egress) |
| `web search` | Web search | Low (read-only network) |
...
| `a concise user question` | Pop a structured question UI | Some agent contexts strip this; prefer free-form questions in the agent's output instead |
...
```yaml
tools: ["Read", "Write", "Edit", "Bash", "Grep", "Glob", "web fetch", "web search"]
```
...
4. **Does it need network access (web docs, external APIs)?** → Add `web fetch` / `web search`.
