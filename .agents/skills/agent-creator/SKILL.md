---
name: agent-creator
description: Create or improve a Codex subagent definition under `.codex/agents/<name>.md`. Use when the user explicitly invokes `/agent-creator`, asks to "make an agent", "create a subagent", "add a new agent", "improve this agent", "tweak the code-reviewer agent", or otherwise wants to author/edit project-scope agent markdown files (with YAML frontmatter for `name`, `description`, `tools`, `model`). Walks the user through a structured interview (open conversation → a concise user question for name / triggering style / tool allow-list / model), writes the agent file, and optionally runs a triggering test via `scripts/test_agent_trigger.py`.
---

# Agent Creator

A skill for authoring and improving Codex **subagents** — the markdown files under `.codex/agents/<name>.md` that define specialized agents launched via the `Task` tool with a `subagent_type` parameter.

This skill is the agent counterpart to `skill-creator`. It only writes to the **project scope** (`.codex/agents/`); user-scope agents (`~/.codex/agents/`) are out of scope.

> Agents vs skills: a **skill** is a workflow Codex itself follows in the main conversation (loaded into context via SKILL.md). An **agent** is a separately-spawned subprocess with its own context, tool allow-list, and model — invoked via the `Task` tool. Use this skill when the user wants to delegate a self-contained task to a fresh sub-context, not to extend the main conversation's behavior.
...
If the user is asking for a **skill** (something Codex follows inline) rather than an agent (something spawned via `Task`), redirect them to `skill-creator` instead.
...
Phase 1: Capture intent (open conversation)
   ↓
Phase 2: Structured hearing (a concise user question for name / trigger / tools / model)
   ↓
Phase 3: Draft and write `.codex/agents/<name>.md`
   ↓
Phase 4: (Optional) Trigger test via scripts/test_agent_trigger.py
```

Phases 1 and 2 can interleave — if the user already supplied some answers in the opening message, skip the corresponding a concise user question fields. Treat this as a conversation, not a form.
...
- **Mode**: New agent vs. improvement of an existing one (check `.codex/agents/` for the target file)
...
- Read the existing `.codex/agents/<name>.md` first
...
Use **a concise user question** for the four fields below. Skip any field already settled in Phase 1. Present all unresolved fields in a single a concise user question call so the user fills them in one screen.
...
Which Codex tools should this agent have? Options to offer:
...
This matches the project-wide policy in `.codex/AGENTS.md`. Do not omit it — the agent runs in its own context and won't inherit the main session's .codex/AGENTS.md awareness automatically.
...
- Avoid duplicating .codex/AGENTS.md content — the agent reads .codex/AGENTS.md when it triggers, so reasserting "use uv, not pip" is redundant.

### Write the file

Use the `Write` tool to create `.codex/agents/<name>.md`. Frontmatter shape:
...
After the file exists, ask via **a concise user question** whether the user wants to verify that the description actually causes the main session to spawn the new agent. This is a quick sanity check, not a full eval.
...
1. Read the existing `.codex/agents/<name>.md` first.
...
- `references/tools.md` — Codex tool name list and allow-list patterns by agent type.
...
- **Re-stating .codex/AGENTS.md inside the agent.** The agent reads the project .codex/AGENTS.md when it spawns. Pasting its rules into every agent body is duplication that goes stale.
