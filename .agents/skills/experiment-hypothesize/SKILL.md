...
If a `hypotheses.md` already exists at `docs/experiment/<family>/{yyyymmdd}_case{N}_{topic}/hypotheses.md`, follow up with one more `a concise user question`: `再構築` (back up existing and reset) / `追記` (append new hypotheses only) / `読み直すだけ` (display current contents and exit).
...
`a concise user question`, 1 round, multi-select with Other free-text. Bucketed by category:
...
Output path (per `.agents/rules/docs.md`):
...
- **All user-facing output, a concise user question labels/descriptions, and the written `hypotheses.md` body must be in Japanese** (per `.codex/AGENTS.md`)
