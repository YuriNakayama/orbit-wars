...
This skill uses an interactive selection-based approach throughout all steps. Every decision point uses `a concise user question` to present choices to the user, making the planning process guided and efficient.
...
3. **Free-text is always available** — The `a concise user question` tool automatically provides an "Other" option for free input. Remind users of this in the first question of each step.
...
Use web search and web fetch to research:
...
Research quality standards:
- External research should include source URLs. Use web search and web fetch to verify claims.
...
Present questions organized by the following perspectives. Ask one round of `a concise user question` (up to 4 questions), then follow up with additional rounds as needed based on answers. The first question should mention: "各質問で「Other」を選ぶと自由記述も可能です。"
...
Build on the existing project architecture (see .codex/AGENTS.md) and include:
...
- Every decision point uses `a concise user question` with `multiSelect: true`. Always present 8+ choices per topic by splitting into multiple questions.
...
- Follow the folder structure, technology stack, and glossary in .codex/AGENTS.md.
- Implementation steps should be granular enough for Codex to execute one at a time later.
