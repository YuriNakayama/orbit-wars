"""runpod_io.runpod.pod_state の正規化と integral helper の単体テスト。"""
...
from runpod_io.runpod import pod_state
...
def test_list_pods_filters_via_sdk(sample_pod_payload: dict[str, Any]) -> None:
    with patch(
        "runpod_io.runpod.pod_state.runpod_sdk.get_pods",
...
def test_list_pods_returns_empty_when_none() -> None:
    with patch("runpod_io.runpod.pod_state.runpod_sdk.get_pods", return_value=None):
        assert pod_state.list_pods() == []


def test_get_pod_returns_none_when_missing() -> None:
    with patch("runpod_io.runpod.pod_state.runpod_sdk.get_pod", return_value=None):
