"""Parity: case_jax aim_with_prediction_jax vs Python aim_with_prediction.
...
from pipeline.rulebase.case_jax.baseline.core.config import HORIZON
...
from pipeline.rulebase.case_jax.baseline_jax.aim_jax import (
    MAX_COMET_PATH_LEN,
...
def test_aim_parity_noncomet(seed: int) -> None:
    ...
    for src, target, ships, ang_vel in pairs:
        ...
        # case8 caches predict_planet_position on (id(initial_by_id), pid, turns);
        # the loop reuses pids 0/1 with short-lived dicts whose id() can collide,
        # so reset per call as agent() does once per turn.
        reset_predict_cache()
