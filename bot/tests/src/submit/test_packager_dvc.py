...
def test_ensure_dvc_artifacts_skips_when_all_outs_exist(tmp_path: Path) -> None:
    repo = _make_fake_repo(tmp_path, outs=["bot/pipeline/case1/policy/weights.pt"])
    case_dir = repo / "bot/pipeline/case1"
...
def test_ensure_dvc_artifacts_invokes_dvc_pull_for_missing(tmp_path: Path) -> None:
    ...
    assert cmd[-1] == "bot/pipeline/case1/policy/weights.pt"
...
def test_ensure_dvc_artifacts_ignores_outs_outside_case_dir(tmp_path: Path) -> None:
    repo = _make_fake_repo(
        ...
        outs=[
            ...
            "bot/pipeline/case1/policy/weights.pt",
        ],
    )
    case_dir = repo / "bot/pipeline/case1"
    case_dir.mkdir(parents=True)
    ...
    assert "bot/pipeline/case1/policy/weights.pt" in cmd
