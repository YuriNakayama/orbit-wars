"""Regression tests: case10 must send larger, fewer fleets than case2.
...
from pipeline.rulebase.case10.baseline import agent as agent_v4  # noqa: E402
...
@pytest.mark.slow
def test_case10_average_fleet_size_larger_than_case2() -> None:
    ...
    assert v4_avg > v2_avg * 1.05, (
        f"case10 avg fleet ({v4_avg:.1f}) should exceed case2 avg ({v2_avg:.1f}) by >5%"
    )


@pytest.mark.integration
def test_case10_has_large_fleet_launches() -> None:
    """case10 should produce at least one fleet of >= 25 ships (FULL_COMMIT effect)."""
    peaks = _max_observed_ships_per_fleet(agent_v4, seed=0, max_turns=100)
    assert peaks, "case10 should launch at least some fleets"
    max_peak = max(peaks.values())
    assert max_peak >= 25, (
        f"case10 should occasionally launch 25+ ship fleets; max observed={max_peak}"
    )
