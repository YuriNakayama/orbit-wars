"""Integration (e2e): case9 JAX port vs real case9 Python agent.

Mirrors case1's test_agent_jax_identity (docs/experiment/rulebase/
20260603_case9_jax_port/plan.md). The #1 risk is the JAX rewrite degrading to a
~0 win-rate; this gate targets it directly with a small (10-game) tripwire rather
than a large eval.

case9 = case8 + anti-ping-pong cooldowns (stateful, but dormant in normal play:
case9 Python == case8 Python 0 mismatch / 200 turns), so case9's JAX port =
case8's core_jax verbatim (the stateless JAX agent can't hold launch history).
The JAX port reuses case1's parity-verified core_jax with case9's config deltas
(PARTIAL_SOURCE_MIN_SHIPS 16, REINFORCE margins, opening mults). OM / lookahead
are default-OFF in case9 config, so the runtime agent reduces to plan_moves with
the forked missions — close to case1's core_jax.

Marked `slow` (full 500-turn games). Run locally on CPU:
    uv run pytest tests/e2e/pipeline/rulebase/case9/test_agent_jax_identity.py -q
"""
...
from pipeline.rulebase.case9.baseline.agent import agent as v9_py
from pipeline.rulebase.case9.baseline_jax.core_jax.agent_full_jax import (
...
def _play_jax_vs_python(seed: int, jax_seat: int) -> int:
    """Play case9 JAX port (jax_seat) vs real case9 Python. Return winner seat or -1."""
    state = reset(seed=seed, num_agents=2)
    py_seat = 1 - jax_seat
    rewards = None
    for _turn in range(500):
        a_jax = compute_actions_jax(state, seat=jax_seat)
        a_py = _py_row(v9_py(state_to_obs(state, player=py_seat)))
...
def test_jax_vs_python_runs_clean(seed: int) -> None:
    """Smoke: case9 JAX port vs real Python runs a full game with no NaN / bad shapes."""
    state = reset(seed=seed, num_agents=2)
    for _turn in range(500):
        a0 = compute_actions_jax(state, seat=0)
        assert a0.shape == (MAX_LAUNCHES_PER_AGENT, 3)
        assert not bool(jnp.any(jnp.isnan(a0)))
        a1 = _py_row(v9_py(state_to_obs(state, player=1)))
...
def test_jax_port_not_catastrophically_worse_than_python() -> None:
    """Anti-regression: case9 JAX port must not collapse to a near-0 win-rate.
    ...
    for seed in range(5):
        for jax_seat in (0, 1):
            ...
            if winner == jax_seat:
                jax_wins += 1
    assert jax_wins >= 3, (
        f"case9 JAX port won {jax_wins}/{games} vs real Python — catastrophic "
