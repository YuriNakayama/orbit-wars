"""Integration (e2e): case7 JAX port vs real case7 Python agent.

Mirrors case1's test_agent_jax_identity (docs/experiment/rulebase/
20260603_case7_jax_port/plan.md). The #1 risk is the JAX rewrite degrading to a
~0 win-rate; this gate targets it directly with a small (10-game) tripwire rather
than a large eval.

case7 = case8-lineage + STAY (t14 ship-loss trap: STAY fires ~60 ships at t14 and
loses ~70%%, memory project_case7_t14_trap). STAY is stateful; the JAX port omits it.
Hypothesis: omitting a self-destructive STAY makes the port STRONGER than case7 Python.
The JAX port reuses case1's parity-verified core_jax with case7's config deltas
(PARTIAL_SOURCE_MIN_SHIPS 16, REINFORCE margins, opening mults). OM / lookahead
are default-OFF in case7 config, so the runtime agent reduces to plan_moves with
the forked missions — close to case1's core_jax.

Marked `slow` (full 500-turn games). Run locally on CPU:
    uv run pytest tests/e2e/pipeline/rulebase/case7/test_agent_jax_identity.py -q
"""
...
from pipeline.rulebase.case7.baseline.agent import agent as v7_py
from pipeline.rulebase.case7.baseline_jax.core_jax.agent_full_jax import (
...
def _play_jax_vs_python(seed: int, jax_seat: int) -> int:
    """Play case7 JAX port (jax_seat) vs real case7 Python. Return winner seat or -1."""
    state = reset(seed=seed, num_agents=2)
    py_seat = 1 - jax_seat
    rewards = None
    for _turn in range(500):
        a_jax = compute_actions_jax(state, seat=jax_seat)
        a_py = _py_row(v7_py(state_to_obs(state, player=py_seat)))
...
def test_jax_vs_python_runs_clean(seed: int) -> None:
    """Smoke: case7 JAX port vs real Python runs a full game with no NaN / bad shapes."""
    state = reset(seed=seed, num_agents=2)
    for _turn in range(500):
        a0 = compute_actions_jax(state, seat=0)
        assert a0.shape == (MAX_LAUNCHES_PER_AGENT, 3)
        assert not bool(jnp.any(jnp.isnan(a0)))
        a1 = _py_row(v7_py(state_to_obs(state, player=1)))
...
def test_jax_port_not_catastrophically_worse_than_python() -> None:
    """Anti-regression: case7 JAX port must not collapse to a near-0 win-rate.
    ...
    for seed in range(5):
        for jax_seat in (0, 1):
            ...
            if winner == jax_seat:
                jax_wins += 1
    assert jax_wins >= 3, (
        f"case7 JAX port won {jax_wins}/{games} vs real Python — catastrophic "
