"""gpu.runpod.cli の wiring + 各サブコマンドのフロー検証 (mock 中心)。"""
...
from gpu.runpod.git import verify_commit_pushed as _verify_commit_pushed
from gpu.runpod.runpod.offers import Offer
...
def test_train_happy_path(
    ...
    sdk = MagicMock()
    monkeypatch.setattr("gpu.runpod.cli.app._build_sdk", lambda _key: sdk)
    monkeypatch.setattr("gpu.runpod.cli.app._volume_sdk", lambda: MagicMock())
    monkeypatch.setattr("gpu.runpod.cli.app.list_volumes", lambda _sdk: [])
    monkeypatch.setattr(
        "gpu.runpod.cli.app.search_offers", lambda _sdk, **_kwargs: [mock_offer]
    )
    monkeypatch.setattr(
        "gpu.runpod.cli.app.pick_offer", lambda offers, console=None: offers[0]
    )
    monkeypatch.setattr(
        "gpu.runpod.cli.app.create_pod",
...
def test_train_no_offers_exits(
    ...
    monkeypatch.setattr("gpu.runpod.cli.app._volume_sdk", lambda: MagicMock())
    monkeypatch.setattr("gpu.runpod.cli.app.list_volumes", lambda _sdk: [])
    monkeypatch.setattr("gpu.runpod.cli.app.search_offers", lambda _sdk, **_kwargs: [])
...
def test_train_cost_limit_aborts_when_declined(
    ...
    monkeypatch.setattr("gpu.runpod.cli.app._build_sdk", lambda _key: MagicMock())
    monkeypatch.setattr("gpu.runpod.cli.app._volume_sdk", lambda: MagicMock())
    monkeypatch.setattr("gpu.runpod.cli.app.list_volumes", lambda _sdk: [])
    monkeypatch.setattr(
        "gpu.runpod.cli.app.search_offers", lambda _sdk, **_kwargs: [expensive]
    )
    monkeypatch.setattr(
        "gpu.runpod.cli.app.pick_offer", lambda offers, console=None: offers[0]
    )
...
def test_pull_runs_dvc_pull(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("gpu.runpod.cli.app._repo_root", lambda: repo_root)
...
def test_pull_missing_dvc_meta_falls_back_to_s3(
    ...
    monkeypatch.setattr(
        "gpu.runpod.execution.progress.list_artifacts",
    ...
    monkeypatch.setattr(
        "gpu.runpod.execution.progress.download_artifact", fake_download
    )
...
def test_pull_invalid_from(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.setattr("gpu.runpod.cli.app._repo_root", lambda: tmp_path)
...
def test_tail_invokes_ssh(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    write_launch: Callable[[Path], None],
) -> None:
    """tail は launch.json から pod_id を取り、SSH 経由 tail コマンドを呼ぶ。"""
    from gpu.runpod.runpod.ssh import SshEndpoint

    repo_root = tmp_path
    run_dir = repo_root / "data/output/models/imitation/case1/runs/run42"
    write_launch(run_dir)
    monkeypatch.setattr("gpu.runpod.cli.app._repo_root", lambda: repo_root)
    monkeypatch.setattr("gpu.runpod.cli.app._ensure_runpod_key", lambda: None)
    monkeypatch.setattr(
        "gpu.runpod.runpod.ssh.get_pod_ssh_endpoint",
...
def test_summary_displays_status_and_artifacts(
    ...
    from gpu.runpod.execution.summary import ArtifactStatus, RunSummary
    ...
    monkeypatch.setattr(
        "gpu.runpod.execution.summary.build_summary", lambda *a, **k: fake
    )
...
def test_tail_ssh_unavailable_exits(
    ...
    from gpu.runpod.runpod.ssh import SshUnavailable
    ...
    monkeypatch.setattr("gpu.runpod.runpod.ssh.get_pod_ssh_endpoint", raise_unavailable)
...
def test_train_writes_launch_json(
    ...
    monkeypatch.setattr("gpu.runpod.cli.app.create_pod", lambda *_a, **_k: "pod-xyz")
    monkeypatch.setattr("gpu.runpod.cli.app._repo_root", lambda: tmp_path)
...
def test_ps_lists_active_pods(monkeypatch: pytest.MonkeyPatch) -> None:
    from gpu.runpod.runpod.pod_state import PodState

    monkeypatch.setattr("gpu.runpod.cli.app._ensure_runpod_key", lambda: None)
    monkeypatch.setattr(
        "gpu.runpod.runpod.pod_state.list_pods",
        lambda: [
            PodState(
                pod_id="pod-abc",
                name="my-run",
...
def test_ps_no_pods(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("gpu.runpod.cli.app._ensure_runpod_key", lambda: None)
    monkeypatch.setattr("gpu.runpod.runpod.pod_state.list_pods", lambda: [])
    result = runner.invoke(app, ["ps"])
    assert result.exit_code == 0
    assert "No active pods" in result.output


def test_status_full_flow(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    write_launch: Callable[[Path], None],
) -> None:
    ...
    monkeypatch.setattr("gpu.runpod.cli.app._ensure_runpod_key", lambda: None)
    monkeypatch.setattr(
        "gpu.runpod.runpod.pod_state.get_pod",
    ...
    monkeypatch.setattr(
        "gpu.runpod.execution.progress.list_markers",
        lambda _run, **_k: [
            ProgressMarker(
...
def test_status_progress_unavailable_warns(
    ...
    monkeypatch.setattr("gpu.runpod.execution.progress.list_markers", boom)
    result = runner.invoke(app, ["status", "run42"])
    assert result.exit_code == 0
...
def test_logs_lists_s3_markers(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    write_launch: Callable[[Path], None],
) -> None:
...
def test_logs_no_markers_exits_with_warning(
    ...
    monkeypatch.setattr("gpu.runpod.cli.app._repo_root", lambda: repo_root)
    monkeypatch.setattr(
        "gpu.runpod.execution.progress.list_markers", lambda run_id, profile=None: []
    )
    result = runner.invoke(app, ["logs", "run42"])
    assert result.exit_code == 1
...
def test_logs_source_onstart_returns_full_log(
    ...
    from gpu.runpod.execution.progress import OnstartLog
    ...
    monkeypatch.setattr("gpu.runpod.execution.progress.fetch_onstart_log", fake_fetch)
...
def test_logs_source_onstart_missing_exits(
    ...
    monkeypatch.setattr("gpu.runpod.cli.app._repo_root", lambda: repo_root)
    monkeypatch.setattr(
        "gpu.runpod.execution.progress.fetch_onstart_log",
