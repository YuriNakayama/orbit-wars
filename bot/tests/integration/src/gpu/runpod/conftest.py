...
from gpu.runpod.auth import AwsCreds
...
def mock_credentials(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "gpu.runpod.cli.app.load_aws_creds",
    ...
    monkeypatch.setattr(
        "gpu.runpod.cli.app.load_runpod_api_key", lambda: "RUNPOD_KEY_FAKE"
    )
...
def mock_git(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "gpu.runpod.cli.app._git_remote_url",
        lambda: "https://github.com/YuriNakayama/orbit-wars.git",
    )
    monkeypatch.setattr(
        "gpu.runpod.cli.app._git_current_branch", lambda: "feature-test"
    )
    monkeypatch.setattr("gpu.runpod.cli.app._verify_commit_pushed", lambda _sha: None)
