"""gpu.runpod.runpod.volumes のユニットテスト。"""
...
def test_create_volume_returns_id(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("gpu.runpod.runpod.volumes.requests.post", fake_post)
...
def test_create_volume_http_error(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "gpu.runpod.runpod.volumes.requests.post", lambda url, **kwargs: FakeResponse()
    )
