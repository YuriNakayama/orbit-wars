...
from gpu.vast.cli import _verify_commit_pushed, app
from gpu.vast.offers import Offer
...
def test_train_happy_path(
    ...
    sdk = MagicMock()
    monkeypatch.setattr("gpu.vast.cli._build_sdk", lambda _key: sdk)
    monkeypatch.setattr("gpu.vast.cli.list_volumes", lambda _sdk: [])
    monkeypatch.setattr(
        "gpu.vast.cli.search_offers", lambda _sdk, **_kwargs: [mock_offer]
    )
    monkeypatch.setattr(
        "gpu.vast.cli.pick_offer", lambda offers, console=None: offers[0]
    )
    monkeypatch.setattr(
        "gpu.vast.cli.create_instance",
        lambda *_a, **_k: 99999,
    )
...
def test_train_no_offers_exits(
    ...
    monkeypatch.setattr("gpu.vast.cli._build_sdk", lambda _key: sdk)
    monkeypatch.setattr("gpu.vast.cli.list_volumes", lambda _sdk: [])
    monkeypatch.setattr("gpu.vast.cli.search_offers", lambda _sdk, **_kwargs: [])
...
def test_train_cost_limit_aborts_when_declined(
    ...
    monkeypatch.setattr("gpu.vast.cli._build_sdk", lambda _key: MagicMock())
    monkeypatch.setattr("gpu.vast.cli.list_volumes", lambda _sdk: [])
    monkeypatch.setattr(
        "gpu.vast.cli.search_offers", lambda _sdk, **_kwargs: [expensive]
    )
    monkeypatch.setattr(
        "gpu.vast.cli.pick_offer", lambda offers, console=None: offers[0]
    )
...
def test_pull_runs_dvc_pull(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("gpu.vast.cli._repo_root", lambda: repo_root)
...
def test_promote_copies_and_updates_status(
    ...
    (repo_root / "bot/pipeline/imitation/case1/policy").mkdir(parents=True)
