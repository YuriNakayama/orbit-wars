"""runpod_io.runpod.volumes のユニットテスト。"""
...
def test_create_volume_returns_id(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("runpod_io.runpod.volumes.requests.post", fake_post)
...
def test_create_volume_http_error(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "runpod_io.runpod.volumes.requests.post", lambda url, **kwargs: FakeResponse()
    )
