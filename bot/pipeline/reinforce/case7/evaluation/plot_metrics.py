"""Plot a case7 training run's metrics.json as learning curves (requirement 3).
...
Usage (from bot/):
    uv run python -m pipeline.reinforce.case7.evaluation.plot_metrics \
        --metrics data/output/models/reinforce/case7_train_jax/runs/<run>/metrics.json \
        --out /tmp/case7_curves.png
"""
...
_OUT_OPT = typer.Option(Path("/tmp/case7_curves.png"), "--out", help="output PNG path")
...
def main(metrics: Path = _METRICS_OPT, out: Path = _OUT_OPT) -> None:
    ...
    fig.suptitle(
        f"case7 run — iters={iters_run} best_win={best:.3f} opponents={opponents}",
        fontsize=12,
    )
