"""Evaluate rl_v0 (reinforce/case0) vs a chosen baseline over N episodes.

CLI mirrors imitation/case1's evaluator. Default opponent is `baseline_v1`
(rulebase/case1) since it's the canonical reference; pass `--baseline random`
for a sanity check that the PPO weights have learned anything at all.
"""
...
app = typer.Typer(
    ...
    help="Evaluate reinforce/case0 PPO vs a chosen baseline.",
)
...
def main(
    ...
    out_path = (
        _abspath(out)
        if out is not None
        else _abspath("data/mart/reinforce/case0/eval_metrics.json")
    )
    spec = RunSpec(
