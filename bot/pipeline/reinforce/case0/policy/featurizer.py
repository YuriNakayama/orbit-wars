"""obs (dict) → BatchFeatures for the reinforce/case0 PPO baseline.

Self-contained per-planet features. Avoids the imitation/case1 template
machinery because PPO selects targets directly over the planet set.
"""
...
def _fleet_speed_proxy(ships: int) -> float:
...
def _fleet_target_eta(
    ...
    speed = _fleet_speed_proxy(ships)
    if speed <= 0:
        return None
    return hit_d / speed
...
def featurize(obs: Any) -> tuple[BatchFeatures, WorldSnapshot]:
    ...
    for fleet_row in raw_fleets:
        _, fowner, fx, fy, fangle, _, fships = fleet_row
        f_owner = int(fowner)
        f_ships = int(fships)
        f_x = float(fx)
        f_y = float(fy)
