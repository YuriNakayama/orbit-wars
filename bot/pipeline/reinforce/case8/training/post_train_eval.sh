...
# Usage:
#   cd bot
#   bash pipeline/reinforce/case8/training/post_train_eval.sh <run_id>
#
# Inputs:
...
#     default: case8_kaggle_jax_train_pool_v1 (matches case_arg in cases.py)
#   $EPISODES (env, optional, default 30): number of eval episodes (local constraint)
#   $BASELINE (env, optional, default baseline_v1)
#
# Outputs:
#   pipeline/reinforce/case8/policy/weights.pt (updated)
#   data/mart/reinforce/case8/eval_metrics.json (eval result + Wilson CI)
#
...
RUN_ID="$1"
CASE_SUBDIR="${POST_TRAIN_CASE_SUBDIR:-case8_kaggle_jax_train_pool_v1}"
EPISODES="${EPISODES:-30}"
BASELINE="${BASELINE:-baseline_v1}"

RUN_DIR="data/output/models/reinforce/${CASE_SUBDIR}/runs/${RUN_ID}"
BEST_PT="${RUN_DIR}/best.pt"
WEIGHTS_OUT="pipeline/reinforce/case8/policy/weights.pt"
...
uv run python -m pipeline.reinforce.case8.training.jax_to_torch \
  --best "$BEST_PT" --out "$WEIGHTS_OUT"

echo "==> eval_vs_baseline: rl_v6 vs $BASELINE x $EPISODES eps"
uv run python -m pipeline.reinforce.case8.evaluation.eval_vs_baseline \
  --episodes "$EPISODES" --baseline "$BASELINE" \
  --label "${RUN_ID}-vs-${BASELINE}"

echo "==> done. metrics: data/mart/reinforce/case8/eval_metrics.json"
