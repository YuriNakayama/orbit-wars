...
class ModelConfigJax:
    ...
    # Subtracted from the launch logit before sampling. The BC prior is
    # heavily no-op-biased, so without a negative offset the launch head
    # would fire with probability ≈ 1.0 and PPO would never see the
    # "do nothing" branch. Mirrors case3's no_op_bias on the NO_OP slot of
    # the (P+1) categorical, now applied to P(Z=1) directly.
    no_op_bias: float = 8.0


class PolicyOutputJax(eqx.Module):
    """3-head factorized policy output.

    Per source planet `s` the action factorizes as
      P(launch, target, ships) = P(Z) · P(Y | Z=1) · P(X | Y, Z=1)
    where
      launch_logit  → P(Z=1) = sigmoid(launch_logit)       (binary launch decision)
      target_logits → P(Y | Z=1) = softmax over P planets  (target, no NO_OP slot)
      ship_mean     → P(X | Y, Z=1) = Normal(ship_mean, σ)  (log1p-ships space)
    """

    launch_logit: jax.Array  # (B, P) — logit of "launch from this source"
    target_logits: jax.Array  # (B, P, P) — per-source target categorical (masked)
...
class PerPlanetHeadJax(eqx.Module):
    """3-head pointer policy: launch (P(Z)) / target (P(Y|Z)) / ships (P(X|Y,Z)).

    Structurally identical projections to case3's 2-head head — the bilinear
    src×tgt pointer (`pair_proj`) now yields the target categorical P(Y|Z)
    *without* the NO_OP slot, and the learned `launch_query` (renamed from
    case3's `noop_query`) is scored against each source query to produce a
    per-source binary launch logit P(Z=1). Ships regression is unchanged.
    """
    ...
    launch_query: jax.Array  # (1, 1, H) — scored against src_q for P(Z=1)
    ...
    def __call__(
        ...
        # Target categorical P(Y | Z=1): per-source × per-target, masked.
        target_logits = jnp.einsum("bph,bqh->bpq", src_q, tgt_kp)
        mask = jnp.broadcast_to(planet_mask[:, None, :], target_logits.shape)
        target_logits = jnp.where(mask, target_logits, jnp.float32(-1e9))
        # Launch logit P(Z=1): per-source scalar = src_q · launch_query.
        launch_q = jnp.broadcast_to(
            self.launch_query, (src_q.shape[0], 1, src_q.shape[-1])
        )
        launch_logit = jnp.einsum("bph,bqh->bpq", src_q, launch_q).squeeze(-1)  # (B, P)
        # Ships regression P(X | Y, Z=1).
        ...
        return launch_logit, target_logits, ship_pred
...
class ActorCriticJax(eqx.Module):
    ...
    def __call__(self, batch: BatchFeaturesJax) -> PolicyOutputJax:
        ...
        launch_logit, target_logits, ship_mean = self.head_per_planet(
            h_with_ctx, batch.planet_mask
        )
        # no_op_bias subtracts from P(Z=1): a positive bias pushes the launch
        # head toward "do nothing", matching case3's NO_OP-slot prior so PPO
        # starts from the BC-like no-op regime and explores firing outward.
        if self.cfg.no_op_bias != 0.0:
            launch_logit = launch_logit - self.cfg.no_op_bias
...
def load_bc_weights_jax(
    ...
    new_head = PerPlanetHeadJax(
        ...
        launch_query=gor(
            "head_per_planet.noop_query", model.head_per_planet.launch_query
        ),
        ship_head_0_w=gor(
