# Reinforce Case4 — 3-head 分解 (launch / target / ship)

`pipeline/reinforce/case4/` は **action head の分解構造**を検証する ablation
ケース。学習レシピ・backbone・featurizer は **case3 と完全同一**で、
**head の確率分解だけ**を 2-head から 3-head に置き換える。

## 仮説

case3 は `(P+1)` カテゴリカルで「どの惑星を target にするか + NO_OP」を一括の
P(Y) として表し、ships を P(X|Y) 回帰とする **2-head 分解**:

```
P(action) = P(Y) · P(X | Y)          # Y ∈ {target_0..target_{P-1}, NO_OP}
```

case4 は「launch するか否か」を独立の 2 値 head に切り出した **3-head 分解**:

```
P(action) = P(Z) · P(Y | Z=1) · P(X | Y, Z=1)
  P(Z)      launch head   — sigmoid(launch_logit)   2値 (撃つ / 撃たない)
  P(Y | Z)  target head   — softmax over P planets  (NO_OP slot なし)
  P(X | Y,Z) ship head    — Normal(ship_mean, σ)    log1p(ships)
```

数学的には同じ族を表現できるが、「行動するか否か」の決定を「どこへ送るか」から
**明示的に分離**することで、PPO の学習信号がよりクリーンになり性能・収束速度が
改善するか、を case3 (2-head) との head-to-head で検証する。

## アーキテクチャ概要 (head 以外は case3 と同一)

- **backbone**: Set Transformer (ISAB×4, hidden=192, attn_heads=8, inducing=24)
- **head_per_planet**: pointer-attention の bilinear src×tgt スコアを `target_logits`
  (P-class、NO_OP slot なし) とし、case3 の `noop_query` を **`launch_query` に改名**
  して per-source の launch logit P(Z=1) を生成。ships head は不変。
- **value_head** (ctx → scalar V) と **ship_log_std** (state-independent Gaussian σ)。
- `no_op_bias` は **launch logit から減算**する prior に転用 (case3 の NO_OP slot
  bias と等価。BC の no-op 偏重から PPO 探索を立ち上げる)。

PPO 損失 = clipped surrogate + value MSE − entropy bonus (本 case は BC OFF /
KL anchor なし、β=0)。log_prob = `log P(Z) + Z·(log P(Y|Z) + log P(X))`。

## 学習レシピ (case3 と同一、head のみ差分)
...
> 学習レシピの実値は `configs/kaggle_jax_train.yaml` を参照。比較 base は同条件で
> 再学習した case3 (2-head)。

> **注意**: 本 case の 3-head 化は **JAX 経路 (`*_jax.py`) のみ**に実装済み。
> PyTorch 経路 (`model.py` / `sampling.py` / `agent.py`) は case3 の 2-head の
> まま (学習は JAX、PyTorch inference 再ロードは follow-up)。

## ディレクトリ構成

```
pipeline/reinforce/case4/
...
uv run python -m pipeline.reinforce.case4.training.train_jax \
    --config pipeline/reinforce/case4/configs/train_jax_smoke.yaml

# 2) 本番学習は RunPod GPU で (kaggle_jax_train.yaml = 200 iter)
git push origin <branch>
dev/runpod train "$(git rev-parse HEAD)" --case case4 --watch

# 3) 評価 (vs baseline, 100 戦)
uv run python -m pipeline.reinforce.case4.evaluation.eval_vs_baseline \
...
uv run python -m pipeline.reinforce.case4.training.train_jax \
  --config pipeline/reinforce/case4/configs/train_jax.yaml
