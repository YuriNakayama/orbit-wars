...
def _bc_kl_jax(
    ...
    across the minibatch, for the 3-head factorization:
      KL = KL(Z) + P_new(Z=1) · [ KL(Y|Z=1) + KL(X|Z=1) ]
    """
    # Launch Bernoulli KL.
    log_p1_new = jax.nn.log_sigmoid(new_out.launch_logit)
    log_p0_new = jax.nn.log_sigmoid(-new_out.launch_logit)
    log_p1_bc = jax.nn.log_sigmoid(bc_out.launch_logit)
    log_p0_bc = jax.nn.log_sigmoid(-bc_out.launch_logit)
    p1_new = jnp.exp(log_p1_new)
    launch_kl = p1_new * (log_p1_new - log_p1_bc) + (1.0 - p1_new) * (
        log_p0_new - log_p0_bc
    )  # (B, P)
    # Target categorical KL (conditional on Z=1).
    log_t_new = jax.nn.log_softmax(new_out.target_logits, axis=-1)
    log_t_bc = jax.nn.log_softmax(bc_out.target_logits, axis=-1)
    p_t_new = jnp.exp(log_t_new)
    tgt_kl = jnp.sum(p_t_new * (log_t_new - log_t_bc), axis=-1)  # (B, P)
    ...
    # Target/ship KL only applies on the launch branch; weight by P_new(Z=1).
    per_kl = launch_kl + p1_new * (tgt_kl + gauss_kl_per)  # (B, P)
    total_kl = jnp.sum(jnp.where(my_mask, per_kl, 0.0), axis=-1)  # (B,)
    return jnp.mean(total_kl)
