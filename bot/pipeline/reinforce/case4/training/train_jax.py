...
PyTorch reinforce/case4 inference path can still load it via the same
...
from pipeline.reinforce.case4.policy.featurizer_jax import (
    GLOBAL_FEAT_DIM,
    PLANET_FEAT_DIM,
)
from pipeline.reinforce.case4.policy.model_jax import (
    ActorCriticJax,
    ModelConfigJax,
    load_bc_weights_jax,
)
from pipeline.reinforce.case4.training.ppo_jax import (
    FlatRollout,
    PPOConfigJax,
    make_optimizer,
    ppo_update_jax,
)
from pipeline.reinforce.case4.training.rollout_jax import (
    JaxRolloutBatch,
    collect_rollout_jax,
)
...
_DEFAULT_CONFIG = typer.Option(
    Path("pipeline/reinforce/case4/configs/train_jax.yaml"),
    "--config",
...
def main(config: Path = _DEFAULT_CONFIG) -> None:
    ...
    # `pipeline/reinforce/case4/configs/foo.yaml` or an absolute path.
