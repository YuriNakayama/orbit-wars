...
class PerPlanetHeadJax(eqx.Module):
    """case2: per_planet pointer + ships + 新規 from_head (発射判断 Bernoulli prior)。

    case1 と同じ pointer-style target categorical + ship_pred regression を
    維持しつつ、per-source の `from_logit` (発射推奨スコア) を追加して
    NO_OP slot に `-from_logit` を加算する。fire が強く推奨されると NO_OP の
    確率が下がり、policy が発射に踏み切りやすくなる。
    """
    ...
    # case2 追加: per-source 発射 prior (B, P, 1)。初期値ゼロで no-op、学習で目覚める。
    from_head_0_w: jax.Array  # (H, 2H)
    from_head_0_b: jax.Array
    from_head_2_w: jax.Array  # (1, H)
    from_head_2_b: jax.Array  # (1,)
    ...
    def from_init(key: jax.Array, hidden: int) -> "PerPlanetHeadJax":
        ...
        return PerPlanetHeadJax(
            ...
            # from_head 初期値: weight は小さく、bias 0 で初期 from_logit ≈ 0
            from_head_0_w=w(keys[8], (h, 2 * h)),
            from_head_0_b=jnp.zeros((h,), dtype=jnp.float32),
            from_head_2_w=w(keys[9], (1, h)),
            from_head_2_b=jnp.zeros((1,), dtype=jnp.float32),
        )
    ...
    def __call__(
        ...
        # case2: from_head — per-source 発射推奨スコア。NO_OP slot に
        # -from_logit を加算することで「発射推奨なら no_op の確率が下がる」。
        from_h = jax.nn.relu(h_with_ctx @ self.from_head_0_w.T + self.from_head_0_b)
        from_logits = from_h @ self.from_head_2_w.T + self.from_head_2_b  # (B, P, 1)
        # NO_OP slot bias: -from_logit (fire 推奨 = no_op 抑制)
        noop_logits_adjusted = noop_logits - from_logits  # (B, P, 1)

        per_planet_logits = jnp.concatenate(
            [pair_logits, noop_logits_adjusted], axis=-1
        )
        # Ships regression.
...
def load_bc_weights_jax(
    ...
    new_head = PerPlanetHeadJax(
        ...
        # case2: from_head は BC checkpoint には存在しないので初期値を維持。
        from_head_0_w=model.head_per_planet.from_head_0_w,
        from_head_0_b=model.head_per_planet.from_head_0_b,
        from_head_2_w=model.head_per_planet.from_head_2_w,
        from_head_2_b=model.head_per_planet.from_head_2_b,
    )
