...
PyTorch reinforce/case6 inference path can still load it via the same
...
from pipeline.reinforce.case6.policy.model_jax import (
    ActorCriticJax,
    ModelConfigJax,
    load_bc_weights_jax,
)
from pipeline.reinforce.case6.training.ppo_jax import (
    FlatRollout,
    PPOConfigJax,
    make_optimizer,
    ppo_update_jax,
...
_DEFAULT_CONFIG = typer.Option(
    Path("pipeline/reinforce/case6/configs/train_jax.yaml"),
    "--config",
...
def main(config: Path = _DEFAULT_CONFIG) -> None:
    ...
    # `pipeline/reinforce/case6/configs/foo.yaml` or an absolute path.
