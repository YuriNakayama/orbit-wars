# Reinforce Case5 — support (補助) reward 検証

`pipeline/reinforce/case5/` は **case3 を丸ごと継承**し、その上に
**support (補助) reward (ship/惑星の保持割合・保持数 等)** を載せて
収束速度・最終性能の向上を検証するケース。head 構造・学習レシピは
case3 と完全同一 (PerPlanetHead、from_head なし、case2 由来の確立レシピ)。

目的:
- 現行報酬 `terminal ±1 + shaping_coef · Δ(mine − enemy)` (shaping_mode は
  ships/planets の排他二択) に対し、ship 差分と planet 差分の同時併用・保持割合
  potential・production potential 等の補助報酬を追加し、reward shaping が
  学習を加速/改善するかを ablation する。
- 仮説リストと skip list は
  `docs/experiment/reinforce/20260527_case5_support_reward/hypotheses.md`。
...
uv run python -m pipeline.reinforce.case5.training.train_jax \
    --config pipeline/reinforce/case5/configs/train_jax_smoke.yaml

# 2) 本番学習は RunPod GPU で (kaggle_jax_train.yaml = 200 iter)
git push origin <branch>
dev/runpod train "$(git rev-parse HEAD)" --case case5 --watch

# 3) 評価 (vs baseline, 100 戦)
uv run python -m pipeline.reinforce.case5.evaluation.eval_vs_baseline \
...
agent registry: `rl_v5` として `bot/src/dataset/selfplay/agents.py` に登録。
...
uv run python -m pipeline.reinforce.case5.training.train_jax \
  --config pipeline/reinforce/case5/configs/train_jax.yaml
