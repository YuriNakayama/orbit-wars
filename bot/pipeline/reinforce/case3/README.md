# Reinforce Case3 — case2 学習レシピ統合 (from_head なし)

`pipeline/reinforce/case3/` は強化学習ファミリの ablation / 統合ベースライン
ケース。**case2 で確立した「from_head 以外の有用な学習レシピ」を全て継承し、
from_head だけを除去**した構成。head 構造は case1 と完全同一 (PerPlanetHead、
from_head なし) で、学習側の工夫のみ case2 から取り込む。

目的は 2 つ:
1. **case2 の from_head 効果の切り分け** — case2 (from_head あり) と case3
   (from_head なし、レシピ同一) を比較し、from_head が勝率を押し上げるかを純粋判定。
2. **学習レシピの統合ベースライン** — case1/case2 実験で効いた工夫を 1 つの
   config に集約した、純正 head の到達点。
