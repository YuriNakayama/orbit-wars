"""Graph U-Net + per-source candidate head for imitation/case4.

Backbone (encoder / TopK pool / decoder / kNN) is **identical** to case3 — the
user constraint is "do not touch the backbone". Only the prediction heads are
redesigned to mirror the Kaggle tutorial action space:

  per-source × CAND_K candidate categorical → argmax slot 0 = no-op.

Head structure follows the notebook's PlanetPolicy:

  - self_h    = MLP(planet h_node) (re-uses the per-node h after decoder)
  - global_h  = MLP(global_feats)
  - cand_h    = MLP(candidate_feats per-slot)
  - logit_k   = MLP([self_h, global_h, cand_h_k]) → 1 (concat per-candidate)

mask: candidate_mask is True for slot 0 always; slots 1..K-1 reflect
"can-fire-here" rule from candidates.py. Invalid slots → logit = -inf.
"""
