"""Parquet → torch Dataset/DataLoader for imitation/case3 IL baseline."""
...
from pipeline.imitation.case3.policy.templates import TEMPLATE_CTX_DIM
...
class CaseThreeDataset(Dataset[Sample]):
    ...
    def __init__(
        ...
        """Load parquet into in-memory tensors.

        `mask_planet_cols` / `mask_global_cols` zero-out the listed feature
        column indices after load — used for ablation studies that strip a
        subset of phase1 features without re-running preprocess.
        """
