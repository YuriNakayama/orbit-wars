"""BC training loop for imitation/case10 candidate/template ship heads."""
...
from pipeline.imitation.case10.policy.model import Case10Policy, ModelConfig
...
def _compute_loss_dispatch(
    ...
    raise ValueError(f"unsupported case10 head_mode={head_mode!r}")
...
def _stamp(msg: str) -> None:
    ...
    """
    logger.info(f"[case10/train] {msg}")
...
def train(cfg: dict[str, Any]) -> TrainReport:
    ...
    head_mode = str(model_cfg.get("head_mode", "candidate_ships"))
    ...
    model = Case10Policy(model_config).to(device)
    _stamp(
    ...
    case_name = str(train_cfg.get("case", "case10"))
