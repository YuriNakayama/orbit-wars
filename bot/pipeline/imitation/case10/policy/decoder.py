"""Policy output → Kaggle action list for imitation/case10."""
...
def _fixed_ship_count(target_ships: int) -> int:
    return max(1, _env_int("IL_CASE10_MIN_SHIP_FLOOR", 5))


def _scale_ship_count(ships: int) -> int:
    scale = _env_float("IL_CASE10_SHIP_SCALE", 1.0)
    if scale >= 1.0:
        return ships
...
def _emit_action(
    ...
    if target.owner != player:
        ...
        if need <= 0:
            return None
        attack_mult = max(1.0, _env_float("IL_CASE10_ATTACK_NEED_MULT", 1.0))
        attack_bonus = max(0, _env_int("IL_CASE10_ATTACK_NEED_BONUS", 0))
        desired = max(need, int(math.ceil(need * attack_mult)) + attack_bonus)
        if ships < desired:
            ships = desired
        overcommit_mult = max(1.0, _env_float("IL_CASE10_ATTACK_OVERCOMMIT_MULT", 2.0))
...
def _decode_candidate(
    ...
    fire_margin = _env_float("IL_CASE10_CAND_FIRE_MARGIN", -0.50)
...
def _decode_template_ships(
    ...
    fire_margin = _env_float("IL_CASE10_TEMPLATE_FIRE_MARGIN", -999.0)
