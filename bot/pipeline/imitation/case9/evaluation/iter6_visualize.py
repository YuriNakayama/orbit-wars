...
Outputs:
- data/output/experiment/imitation_case8_iter6_grouped_importance.json
...
CONFIG = BOT_ROOT / "pipeline/imitation/case8/configs/il_case8.yaml"
WEIGHTS = BOT_ROOT / "pipeline/imitation/case8/policy/weights.pt"
RUN_DIR_BASE = REPO_ROOT / "data/output/models/imitation/case8/runs"
...
def render_panel(importance: ImportanceReport) -> Path:
    ...
    out = OUT_DIR / "imitation_case8_iter6_analysis.png"
...
def render_loss_comparison() -> Path:
    ...
    fig.suptitle(
        "case8 iter1 -> iter6 training comparison\n" + "\n".join(txt_lines),
        fontsize=10,
        family="monospace",
        y=1.02,
    )
    ...
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    out = OUT_DIR / "imitation_case8_iter_loss_comparison.png"
    fig.tight_layout()
    fig.savefig(out, dpi=110, bbox_inches="tight")
    plt.close(fig)
    return out
...
def main() -> None:
    ...
    if not skip_importance:
        ...
        json_out = OUT_DIR / "imitation_case8_iter6_grouped_importance.json"
