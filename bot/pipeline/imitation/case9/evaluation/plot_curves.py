"""Plot training curves for imitation/case8 best iter (iter6, canonical).
...
DEFAULT_OUT = Path("data/output/experiment/imitation_case8_learning_curves.png")
...
def render(log: Path, out: Path, label: str) -> tuple[Path, dict[str, float]]:
    ...
    fig.suptitle(
        f"imitation/case8 — {label} (vs baseline_v1: 0/100)",
        fontsize=13,
    )
    fig.tight_layout()
