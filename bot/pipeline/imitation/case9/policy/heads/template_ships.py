...
is predicted with the same 4-bucket categorical head used by `three_head` and
`candidate_ships`.

Inputs from backbone are the same as `ThreeHead` except `my_planet_mask` is not
needed for forward-time masking.  Loss/decode decide which source rows are used.
