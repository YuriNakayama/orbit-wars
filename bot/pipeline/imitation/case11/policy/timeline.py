...
def summarize_timeline(
    ...
    *,
    ...
    Returns:
        loss_3turn: 自所有 planet なら turn=0 → turn=short_window で失う ships 量
...
def summarize_timeline_multi_horizon(
    ...
    Returns 各 horizon h について:
      ...
      min_owned_{h}turn: turn=0..h で自所有期間中の最小 ships。自所有でなければ 0。
    """
    ships_at = timeline.get("ships_at", {})
    owner_at = timeline.get("owner_at", {})
    ...
    for h in horizons:
        ...
        # min_owned for this horizon: walk turns 0..h, count only when
        # owner == base_owner (mirrors simulate_planet_timeline's min_owned;
        # base_owner is the player-perspective owner at t=0).
        if base_owner == -1:
            mo = 0.0
        else:
            mo_val = (
