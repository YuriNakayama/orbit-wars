"""Plot training curves for imitation/case7 best iter (iter6, canonical).
...
Usage:
    uv run python -m pipeline.imitation.case7.evaluation.plot_curves \\
...
def render(log: Path, out: Path, label: str) -> tuple[Path, dict[str, float]]:
    ...
    fig.suptitle(
        f"imitation/case7 — {label} (vs baseline_v1: 0/100)",
        fontsize=13,
    )
    fig.tight_layout()
