...
uv run python -m pipeline.imitation.case7.evaluation.eval_vs_baseline --episodes 100 --seed 0
```
...
```bash
uv run pytest tests/pipeline/imitation/case7 -v -m "not slow"
uv run pytest tests/pipeline/imitation/case7 -v   # determinism 含む
```

## 設計原則

- **case 間独立**: `pipeline/imitation/case7/` は `pipeline/rulebase/case[012]/` に依存しない / されない。
