"""Replay → parquet preprocess for imitation/case7 IL baseline.
...
from pipeline.imitation.case7.policy.geometry import Planet, aim_with_prediction
from pipeline.imitation.case7.policy.templates import (
    T_NO_OP,
...
def _load_params(config_path: Path | None = None) -> dict[str, Any]:
    if config_path is None:
        ...
        if not path.is_absolute():
            ...
            # `pipeline/imitation/case7/configs/il_case7.yaml` のような cwd
...
def main(
    config: Path = typer.Option(
        ...
        "--config",
        "-c",
        help="YAML config path (default=params.yaml, case5 は configs/il_case7.yaml)",
    ),
) -> None:
