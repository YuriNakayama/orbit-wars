"""obs (dict) -> BatchFeatures (torch.Tensor) for imitation/case2.

Extends case1 with engineered planet/global features. Pure function,
no torch.nn / autograd / random state. Used by both training preprocess
and runtime agent inference.

Planet feature columns (PLANET_FEAT_DIM = 18):
  0  x_norm                 x / BOARD_SIZE
  1  y_norm                 y / BOARD_SIZE
  2  radius_norm            radius / 5
  3  ships_log              log1p(ships)
  4  production_log         log1p(production)
  5  is_mine
  6  is_enemy
  7  is_neutral
  8  is_comet
  9  net_incoming_log       log1p(enemy_incoming) - log1p(my_incoming)
  10 nearest_eta_norm       min(turns) / HORIZON
  11 sun_dist_norm          dist((x,y), sun_center) / sqrt(2)*BOARD_SIZE
  12 is_static              static (rotation-locked) planet flag
  13 prod_per_ship          production / max(1, ships)  (clipped to [0,5]/5)
  14 nearest_enemy_dist     min dist to any enemy planet / diag
  15 support_density        log1p(friendly ships within 25u) / 6
  16 threat_density         log1p(enemy ships+fleets within 25u) / 6
  17 net_incoming_signed    (enemy_inc - my_inc) / max(1, ships)  (clipped)

Global feature columns (GLOBAL_FEAT_DIM = 11):
  0  step_norm              step / 500
  1  ang_vel_scaled         angular_velocity * 10
  2  my_total_ships_log
  3  enemy_total_ships_log
  4  neutral_total_ships_log
  5  prod_diff_log          log1p(my_prod) - log1p(enemy_prod)
  6  my_planet_frac         my_planets / total_planets
  7  enemy_planet_frac      enemy_planets / total_planets
  8  comet_active           1 if step in any comet wave window
  9  phase_mid              1 if 100 <= step < 300
  10 phase_late             1 if step >= 300
"""
...
def featurize(obs: Any) -> tuple[BatchFeatures, WorldSnapshot]:
    ...
    # Pre-compute neighbour densities and nearest-enemy distance per slot.
    support_density = [0.0] * MAX_PLANETS
    threat_density = [0.0] * MAX_PLANETS
    nearest_enemy_dist = [DIAG] * MAX_PLANETS
    for i in range(n):
        _, _owner_i, px_i, py_i, _, _, _ = raw_planets[i]
        for j in range(n):
            if i == j:
                continue
            ...
            if d <= NEIGHBOR_RADIUS:
                if owner_jj == player:
                    ...
                    threat_density[i] += float(ships_j)
            if owner_jj != player and owner_jj != -1 and d < nearest_enemy_dist[i]:
    ...
    for slot in range(n):
        ...
        net_signed = (incoming[slot][1] - incoming[slot][0]) / float(max(1, ships_i))
        feats = [
            float(px) / BOARD_SIZE,
            float(py) / BOARD_SIZE,
            float(radius) / 5.0,
            ...
            math.log1p(support_density[slot]) / 6.0,
            math.log1p(threat_density[slot]) / 6.0,
