"""Post-model safety filters for fleet trajectories (case5 copy).

Mirrors `bot/src/utils/trajectory_safety.py`. Duplicated here so the case
remains self-contained for Kaggle submission packaging — `bot/src/utils/`
is not bundled into the tar.gz. Update the canonical source first, then
sync this file (the cross-case independence rule mandates the copy).
...
def intercept_holds_within_tolerance(
    ...
    initial_by_id: dict[int, SafetyPlanet],
    ang_vel: float,
