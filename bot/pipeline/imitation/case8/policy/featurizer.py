"""obs (dict) → BatchFeatures (torch.Tensor) for imitation/case8 IL baseline.
...
def featurize(
    ...
    for slot in range(n):
        ...
        feats = [
            ...
            # case8 iter1: predicted-distance 2 列
            fut_dist_my,
            fut_dist_enemy,
            # case8 iter1: history 3 列 (obs_{N-2} / obs_{N-3})
            delta_t1,
            delta_t2,
            owner_changed_t1,
            # case8 iter4: G6 (enemy ship-event per-planet) 削除済 ← 元 idx 22-23
            # case8 iter2: fleet trajectory 4 列 (earliest enemy inbound, 5-turn future delta)
            inbound_fleet_dx[slot],
            inbound_fleet_dy[slot],
            inbound_fleet_dist[slot],
            inbound_fleet_ships_log[slot],
            # case8 iter2: multi-horizon ship-prediction 4 列 (h=5/15)
            ...
            # case8 iter2: home distance + production centroid distance 2 列
            home_dist_log,
            prod_centroid_dist_log,
            # case8 iter3 A (iter4 K1 で縮小): Pairwise Top-K (K=3 × 4 feat = 12 列)
            *topk_flat,
            # case8 iter3 D: Defense surplus (4 列、h=5/15/30 + now)
            defense_surplus_5turn,
            defense_surplus_15turn,
            defense_surplus_30turn,
            defense_margin_now,
            # case8 iter3 J: Sparse mask flags (5 列)
            ...
            # case8 iter4 K3: Frontline distance (4 列、敵 nearest 1/2)
    ...
    global_feats = torch.tensor(
        [
            ...
            # case8 iter1: enemy/ally launch history 4 列 (H3)
