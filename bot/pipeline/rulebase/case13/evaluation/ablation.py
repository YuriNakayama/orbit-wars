"""Ablation helper: toggle a case13 config flag, run N games vs case2, report v4 win rate."""
...
CONFIG_PATH = Path("pipeline/rulebase/case13/baseline/core/config.py")
...
def _win_rate(path: Path) -> tuple[int, int, int, int]:
    ...
    for row in df.iter_rows(named=True):
        ...
        if winner_name == "baseline_v13":
            v4_wins += 1
        else:
            v2_wins += 1
    return v4_wins, v2_wins, draws, len(df)
...
def ablate(
    ...
    """Disable FLAG, run games (both seat orders), report v4 win rate, restore."""
    original_value = True
    try:
        _set_flag(flag, False)
        typer.echo(f"[ablate] disabled {flag}")

        run_a = _run(["baseline_v13", "baseline_v2"], episodes_per_side, seed, parallel)
        run_b = _run(
            ["baseline_v2", "baseline_v13"], episodes_per_side, seed + 500, parallel
        )
        ...
        total_n = na + nb
        typer.echo(
            f"[ablate] flag={flag} v4 wins {total_w}/{total_n} "
...
def baseline_run(
    ...
    """Run v4 (current config) vs v2 both directions and report."""
    run_a = _run(["baseline_v13", "baseline_v2"], episodes_per_side, seed, parallel)
    run_b = _run(
        ["baseline_v2", "baseline_v13"], episodes_per_side, seed + 500, parallel
    )
    ...
    wb, lb, db, nb = _win_rate(run_b)
    typer.echo(
        f"[baseline] v4 wins {wa + wb}/{na + nb} "
