...
from .missions.stay import build_stay_holds
...
def plan_moves(
...
) -> tuple[list[list[int | float]], set[int]]:
    """Plan one turn of moves.

    Returns (moves, burst_held_src_ids). The second element contains the source
    planet ids that were burst-held this turn — callers can use it to track
    consecutive-hold counts for ``STAY_BURST_MAX_HOLD_TURNS`` enforcement.
    """
    ...
    def source_attack_left(source_id: int) -> int:
        ...
        held = stay_holds.get(source_id, 0)
    ...
    return _enforce_inventory_cap(moves, world), burst_held_src_ids
