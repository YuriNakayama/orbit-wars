...
from .stay_jax import burst_held_mask
...
def build_world_features(obs: Any) -> WorldFeatures:
    ...
    # case6 STAY burst-hold: zero a source's attack budget when waiting one turn
    # lets the next launch arrive earlier (mirror of baseline source_attack_left
    # = raw - held, where held == usable for a burst-held source).
    _burst_held = burst_held_mask(
        jnp.stack([xs_j, ys_j], axis=-1),
        radius_j,
        owner_j,
        valid_j,
        available,
    ...
    available = jnp.where(_burst_held, jnp.int32(0), available).astype(jnp.int32)
...
def build_world_features_from_state(state: EnvState, seat: int) -> WorldFeatures:
    ...
    # case6 STAY burst-hold (see build_world_features): zero burst-held sources.
    _burst_held = burst_held_mask(
        jnp.stack([xs, ys], axis=-1), radius, owner, valid, available, prod, player
    )
    available = jnp.where(_burst_held, jnp.int32(0), available).astype(jnp.int32)
