"""Regenerate the action snapshot used by tests/pipeline/rulebase/case10/.

Capture/write helpers live in `src/evaluation/snapshot_update.py`; this wrapper
binds them to the case10 baseline agent and the case10 snapshot paths.
"""
...
from pipeline.rulebase.case10.baseline import agent as baseline_agent
from utils.repo_root import find_repo_root

app = typer.Typer(add_completion=False, help="Snapshot updater for case10 baseline.")

REPO_ROOT = find_repo_root(Path(__file__))
DEFAULT_SNAPSHOT_DIR = (
    REPO_ROOT / "tests" / "pipeline" / "rulebase" / "case10" / "snapshots"
)
