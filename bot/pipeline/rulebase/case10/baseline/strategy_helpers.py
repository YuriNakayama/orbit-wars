...
from .core.config import (
    ...
    SWARM_SCORE_MULT,
    SWARM_VALUE_MULT,
    THRASH_REPEAT_FILTER_ENABLED,
    THRASH_REPEAT_LIMIT,
    THRASH_REPEAT_SCORE_MULT,
...
def apply_score_modifiers(
    ...
    # iter3 thrash repeat filter: same-target capture/snipe/swarm が直近
    # THRASH_REPEAT_WINDOW に LIMIT 回以上 commit 済なら score を MULT 倍に減衰。
    if THRASH_REPEAT_FILTER_ENABLED and mission in ("capture", "snipe", "swarm"):
        commits = world.mission_commits.get(target.id, ())
        cutoff = world.step - THRASH_REPEAT_WINDOW
        recent_count = sum(1 for t in commits if t >= cutoff)
        if recent_count >= THRASH_REPEAT_LIMIT:
            score *= THRASH_REPEAT_SCORE_MULT
    return score
