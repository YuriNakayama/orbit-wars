...
@dataclass
class StayState:
    """Per-game rolling state for STAY judge + iter3 thrash repeat filter."""

    consecutive_holds: dict[int, int] = field(default_factory=dict)
    accumulate_holds: dict[int, int] = field(default_factory=dict)
    last_step: int | None = None
    # iter3: planet_id -> [step at which self committed capture/snipe/swarm]
...
def record_mission_commit(planet_id: int, step: int) -> None:
    """Record a capture/snipe/swarm mission commit for thrash repeat filter.

    Called from `strategy._process_*_mission` after a successful move append.
    Keeps the list bounded by pruning entries older than THRASH_REPEAT_WINDOW.
    """
    cutoff = step - THRASH_REPEAT_WINDOW
    bucket = _STAY_STATE.mission_commits.setdefault(planet_id, [])
    bucket.append(step)
    if bucket and bucket[0] < cutoff:
        _STAY_STATE.mission_commits[planet_id] = [t for t in bucket if t >= cutoff]
