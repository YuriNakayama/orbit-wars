...
def _process_single_source_mission(
    ...
    # iter3 thrash repeat filter: record this commit for capture-like
    # missions (single = capture, snipe). Skip reinforce/harass/crash_exploit/
    # accumulate_fire which target the same planet for different purposes.
    if mission.kind in ("single", "snipe"):
        from .agent import record_mission_commit

        record_mission_commit(target.id, world.step)
...
def _process_multi_source_mission(
    ...
    # iter3 thrash repeat filter: multi-source missions are swarm = capture-like.
