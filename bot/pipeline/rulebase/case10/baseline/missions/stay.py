...
def _accumulate_target_threshold(
    ...
    iter4: need-based 動的化。floor_threshold = max(need + SAFETY, FLOOR=30)、
    overshoot_cap = max(need + SAFETY * 2, need * OVERSHOOT_RATIO=1.5) で
    上限を抑える。small need の target に 60 ships を一斉発射する旧設計の
    過剰発射 (30戦敗北分析の主因) を解消。
    """
    need = world.ships_needed_to_capture(target_id, arrival_turn, planned_commitments)
    floor_threshold = max(
        need + cfg.ACCUMULATE_SAFETY_SHIPS, cfg.ACCUMULATE_KNEE_SHIPS_FLOOR
    )
    overshoot_cap = max(
...
def _build_accumulate(
    ...
    # case10 t14 罠対策: 序盤 step では accumulate を発動しない。
    if world.step < cfg.ACCUMULATE_MIN_LAUNCH_STEP:
        return holds, fire_missions, held_ids
