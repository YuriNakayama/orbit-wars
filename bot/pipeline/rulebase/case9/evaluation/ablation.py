"""Ablation helper: toggle a case9 config flag, run N games vs case2, report v9 win rate."""
...
CONFIG_PATH = Path("pipeline/rulebase/case9/baseline/core/config.py")
...
def _win_rate(path: Path) -> tuple[int, int, int, int]:
    ...
    for row in df.iter_rows(named=True):
        ...
        if winner_name == "baseline_v9":
            v9_wins += 1
        else:
            v2_wins += 1
    return v9_wins, v2_wins, draws, len(df)
...
def ablate(
    ...
    """Disable FLAG, run games (both seat orders), report v9 win rate, restore."""
    original_value = True
    try:
        _set_flag(flag, False)
        typer.echo(f"[ablate] disabled {flag}")

        run_a = _run(["baseline_v9", "baseline_v2"], episodes_per_side, seed, parallel)
        run_b = _run(
            ["baseline_v2", "baseline_v9"], episodes_per_side, seed + 500, parallel
        )
        ...
        total_n = na + nb
        typer.echo(
            f"[ablate] flag={flag} v9 wins {total_w}/{total_n} "
...
def baseline_run(
    ...
    """Run v9 (current config) vs v2 both directions and report."""
    run_a = _run(["baseline_v9", "baseline_v2"], episodes_per_side, seed, parallel)
    run_b = _run(
        ["baseline_v2", "baseline_v9"], episodes_per_side, seed + 500, parallel
    )
    ...
    wb, lb, db, nb = _win_rate(run_b)
    typer.echo(
        f"[baseline] v9 wins {wa + wb}/{na + nb} "
