    ...
    for target in world.enemy_planets:
        ...
        if ANTI_PING_PONG_ENABLED and not bypass_cooldown:
            last_to_target = max(
                (
                    last
                    for (_src, dst), last in world.recent_dispatches.items()
                    if dst == target.id
                ),
                default=None,
            )
            if (
                last_to_target is not None
                and world.step - last_to_target < HARASS_TARGET_COOLDOWN_TURNS
            ):
                continue
