...
HARASS_MIN_TARGET_PRODUCTION: int = 3
...
# --- case9 anti planet-ping-pong ---
# Master switch. When False, case9 ≡ case4 (all cooldowns / deficit guards skip).
ANTI_PING_PONG_ENABLED: bool = True
# Reinforce: skip if same (src, dst) pair fired within this many turns.
PING_PONG_PAIR_COOLDOWN_TURNS: int = 1
# Harass: skip if same target was harass-targeted within this many turns.
HARASS_TARGET_COOLDOWN_TURNS: int = 2
# Reinforce: drop threatened candidates whose deficit is below this floor.
# iter2: relaxed back to 1 (analysis shows MIN_DEFICIT=3 starves legitimate
# small-threat reinforces in mid-game). The remaining filter is production-based.
REINFORCE_MIN_DEFICIT: int = 1
# When the agent has this many planets or fewer, bypass cooldowns entirely.
# Rationale (iter1_analysis.md): in collapse phase the src pool shrinks so
