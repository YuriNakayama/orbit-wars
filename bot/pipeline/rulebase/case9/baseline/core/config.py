...
# ----- case9: planet thrash filter (case8 iter3 v1 から移植) -----
# 直近 THRASH_WINDOW ターン以内に self が他 owner に奪われた planet への
# capture/snipe/swarm mission の score を THRASH_SCORE_MULT 倍に減衰。
# REPEAT_LIMIT=999 で mission_commits 経路は無効化、recently_lost 単独で判定。
# result.md (2026-05-05) で filter は case4 base に対し -10pp の害が判明、撤退。
# default は False (case9 ≡ case4)、改修して再評価する際は True に戻す。
THRASH_FILTER_ENABLED: bool = False
