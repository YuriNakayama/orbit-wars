"""Multi-source swarm mission builder (2- and 3-source synchronized attacks)."""
...
def build_swarm_missions(
    ...
    """Pair/triple top capture options per target into synchronized attacks."""
        ...
        for i in range(len(top_options)):
            for j in range(i + 1, len(top_options)):
                ...
                if abs(first.turns - second.turns) > eta_tolerance_2:
                    continue
                if (
                    first.send_cap < SWARM_MIN_PARTICIPANT_SHIPS
                    or second.send_cap < SWARM_MIN_PARTICIPANT_SHIPS
                ):
                    continue
                ...
                for j in range(i + 1, len(top_options)):
                    for k in range(j + 1, len(top_options)):
                        ...
                        if len(src_ids) < 3:
                            continue
                        if any(
                            opt.send_cap < SWARM_MIN_PARTICIPANT_SHIPS for opt in trio
                        ):
                            continue
