...
from .rollout import rollout_filter, rollout_reorder
...
def plan_moves(world: WorldModel, light: bool = False) -> list[list[int | float]]:
    ...
    if ROLLOUT_ENABLED:
        missions = rollout_reorder(missions, world)
    if ROLLOUT_FILTER_ENABLED:
        missions = rollout_filter(missions, world)
