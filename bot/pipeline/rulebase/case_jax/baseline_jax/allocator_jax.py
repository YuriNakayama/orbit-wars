...
from ..baseline.core.config import HORIZON, REINFORCE_SAFETY_MARGIN
...
def run_single_source_allocator(
    ...
    """
    # Sort by -score; invalid rows pushed to the bottom via -inf score.
    eff_score = jnp.where(table.valid, table.score, jnp.float32(-jnp.inf))
    order = jnp.argsort(-eff_score)  # (N,) descending

    # Carry: per-target commit ledger, per-source spent, move buffer.
    init_commit_eta = jnp.zeros((MAX_PLANETS, MAX_COMMIT), dtype=jnp.int32)
    init_commit_ships = jnp.zeros((MAX_PLANETS, MAX_COMMIT), dtype=jnp.int32)
    init_commit_count = jnp.zeros((MAX_PLANETS,), dtype=jnp.int32)
    init_spent = jnp.zeros((MAX_PLANETS,), dtype=jnp.int32)
    init_move_src = jnp.full((MAX_MOVES,), -1, dtype=jnp.int32)
    init_move_angle = jnp.zeros((MAX_MOVES,), dtype=jnp.float32)
    init_move_ships = jnp.zeros((MAX_MOVES,), dtype=jnp.int32)
    init_move_count = jnp.int32(0)
    ...
    def step(carry: Carry, idx: jax.Array) -> tuple[Carry, None]:
        (
            commit_eta,
            commit_ships,
            commit_count,
        ...
        missing = _need_with_commits(
            features, tgt, cutoff, commit_eta[tgt], commit_ships[tgt]
        )
        ...
        # --- commit (masked) ---
        new_spent = spent.at[src].add(jnp.where(ok, sent, 0))

        m_slot = jnp.where(ok, move_count, MAX_MOVES - 1)
        new_move_src = move_src.at[m_slot].set(
            jnp.where(ok, features.planet_id[src], move_src[m_slot])
        )
        new_move_angle = move_angle.at[m_slot].set(
            jnp.where(ok, angle, move_angle[m_slot])
        )
        new_move_ships = move_ships.at[m_slot].set(
            jnp.where(ok, sent, move_ships[m_slot])
        )
        new_move_count = move_count + jnp.where(ok, jnp.int32(1), jnp.int32(0))

        # commit ledger: append (cutoff turn? no — option.turns) for this target.
        c_slot = jnp.where(ok, commit_count[tgt], MAX_COMMIT - 1)
        new_commit_eta = commit_eta.at[tgt, c_slot].set(
            jnp.where(ok, turns, commit_eta[tgt, c_slot])
        )
        new_commit_ships = commit_ships.at[tgt, c_slot].set(
            jnp.where(ok, sent, commit_ships[tgt, c_slot])
        )
        new_commit_count = commit_count.at[tgt].add(
    ...
    final_carry, _ = jax.lax.scan(step, init, order)
    (_ce, _cs, _cc, _sp, move_src, move_angle, move_ships, move_count) = final_carry
    return AllocResult(
        move_src=move_src,
