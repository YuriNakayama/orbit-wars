"""Head-to-head evaluation: baseline_v5 vs baseline_v4.

Runs N episodes in each seat order and reports win rate. Mirrors the
structure of `pipeline.rulebase.case4.evaluation.compare_v2` for consistency.
"""
...
def _run_episode(v5_seat: int, seed: int) -> EpisodeStats:
    ...
    if rewards[0] != rewards[1]:
        ...
        winner = 5 if seat_winner == v5_seat else 4
        ...
        for ep_idx in range(episodes_per_side):
            ...
            if (ep_idx + 1) % 10 == 0:
                typer.echo(
                    ...
                    f"v5={v5_wins} v4={v4_wins} draws={draws}"
                )

    n = v5_wins + v4_wins + draws
    typer.echo("\n=== Summary ===")
