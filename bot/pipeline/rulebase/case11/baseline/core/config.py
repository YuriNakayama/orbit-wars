...
# ----- case11 Portfolio Search family -----
# iter1: Portfolio Greedy Search (Churchill & Buro 2013)
# 各 source planet に script を hill climbing で割り当て、評価関数で探索する。
# iter1 v0/v1/v2/v3 すべて 0% 完敗 (4 連敗)。PGS の構造問題 (1 source 1 script
# モデルが case4 の rich mission set と相性悪い) が確定、撤退。
# OFF で case4 等価動作 (回帰テスト保証)。
PORTFOLIO_ENABLED: bool = False
# Hill climbing: 各 source 1 回ずつ × N ラウンド改善を繰り返す。
PORTFOLIO_HILL_CLIMB_ITERS: int = 3
# Forward simulation 深さ (turns)。長いほど精度高、短いほど高速。
# v0 で horizon=8 だと遠距離 capture (eta>8) が評価されず PGS が idle に固定された。
# 典型 ETA 10-20 を踏まえ horizon=20 に拡張。
PORTFOLIO_PLAYOUT_HORIZON: int = 20
# 1 ターン全体の探索時間上限 (秒)。超過時は best-so-far で early stop。
PORTFOLIO_TIME_BUDGET_S: float = 0.6

# iter2: Nested-Greedy Search (Moraes 2018) — iter2 で True に
NGS_ENABLED: bool = False
NGS_INNER_HILL_CLIMB_ITERS: int = 2

# iter3: Naïve MCTS / CMAB (Ontañón 2017) — iter3 で True に
NAIVE_MCTS_ENABLED: bool = False
