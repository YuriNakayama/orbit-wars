...
# ----- case12 Naïve MCTS / CMAB sampling (Ontañón 2017) -----
# 各 source に script を UCB1 で stochastic sampling、組合せを rollout 評価
# する。case11 PGS の deterministic hill climb 制約 (local optimum 0%) を
# sampling で bypass する試行。
# iter1 v0 で 0/10 完敗 (PGS と同じ「script-only モデル」構造問題)、撤退。
# OFF で case4 等価動作 (回帰テスト保証)。
NAIVE_MCTS_ENABLED: bool = False
NAIVE_MCTS_ROLLOUTS: int = 64
NAIVE_MCTS_EXPLORATION: float = 1.41  # sqrt(2)
NAIVE_MCTS_PLAYOUT_HORIZON: int = 20
NAIVE_MCTS_TIME_BUDGET_S: float = 0.6
