...
from .core.config import (
    NAIVE_MCTS_ENABLED,
    NAIVE_MCTS_EXPLORATION,
    NAIVE_MCTS_PLAYOUT_HORIZON,
    NAIVE_MCTS_ROLLOUTS,
    NAIVE_MCTS_TIME_BUDGET_S,
...
from .planner import run_naive_mcts
...
def plan_moves(world: WorldModel) -> list[list[int | float]]:
    if NAIVE_MCTS_ENABLED:
        # iter1: NaïveMCTS / CMAB sampling delegates entire move selection
        # to per-source script assignment + UCB1 sampling.
        return run_naive_mcts(
            world,
            rollouts=NAIVE_MCTS_ROLLOUTS,
            horizon=NAIVE_MCTS_PLAYOUT_HORIZON,
            exploration_c=NAIVE_MCTS_EXPLORATION,
            time_budget_s=NAIVE_MCTS_TIME_BUDGET_S,
        )
