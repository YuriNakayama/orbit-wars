...
The discovery walks parents of `__file__` looking for the `(bot/, .git)`
...
def find_repo_root(start: Path) -> Path:
    """Walk parents of `start` until a directory with `bot/` and `.git/` is found.

    Raises RuntimeError if no such directory is found.
    """
    here = start.resolve()
    for candidate in (here, *here.parents):
        if (candidate / "bot").is_dir() and (candidate / ".git").exists():
            return candidate
