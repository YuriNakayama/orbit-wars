...
from pipeline.rulebase.case4.baseline.core.physics import fleet_speed
...
def _planet_by_id(obs: dict, pid: int) -> list | None:
...
def _fleet_by_id(obs: dict, fid: int) -> list | None:
...
def _fire_one_fleet(
    env, player_seat: int, from_pid: int, angle: float, ships: int
) -> tuple[dict, int]:
