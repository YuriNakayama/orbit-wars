...
from pipeline.rulebase.case6.baseline.agent import build_world  # noqa: E402
from pipeline.rulebase.case6.baseline.core.physics import travel_time  # noqa: E402
...
def _seeded_obs(seed: int, step: int = 30) -> dict[str, Any]:
    ...
    def noop(_obs: dict[str, Any]) -> list[list[int | float]]:
        return []
    ...
    for _ in range(step):
        if env.done:
            break
        env.step([noop(s["observation"]) for s in env.steps[-1]])
