...
from runpod_io.notify import _osascript_command, notify
...
def test_notify_macos_invokes_osascript(
    ...
    monkeypatch.setattr("runpod_io.notify.sys.platform", "darwin")
    monkeypatch.setattr("runpod_io.notify.shutil.which", lambda _: "/usr/bin/osascript")
    ...
    monkeypatch.setattr("runpod_io.notify.subprocess.run", fake_run)
...
def test_notify_linux_invokes_notify_send(
    ...
    monkeypatch.setattr("runpod_io.notify.sys.platform", "linux")
    monkeypatch.setattr(
        "runpod_io.notify.shutil.which", lambda _: "/usr/bin/notify-send"
    )
    captured: list[list[str]] = []
...
def test_notify_falls_back_to_stdout_when_no_tool(
    ...
    monkeypatch.setattr("runpod_io.notify.sys.platform", "darwin")
    monkeypatch.setattr("runpod_io.notify.shutil.which", lambda _: None)
