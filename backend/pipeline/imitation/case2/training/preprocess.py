"""Replay → parquet preprocess for imitation/case2 IL baseline.
...
from pipeline.imitation.case2.policy import featurizer as featurizer_baseline
from pipeline.imitation.case2.policy import featurizer_phase1
from pipeline.imitation.case2.policy.featurizer import MAX_PLANETS
from pipeline.imitation.case2.policy.geometry import Planet, aim_with_prediction
from pipeline.imitation.case2.policy.templates import (
    T_NO_OP,
    classify_actual_target,
)

FEATURIZERS = {
    "baseline": featurizer_baseline,
    "phase1": featurizer_phase1,
}
...
def preprocess(cfg: dict[str, Any]) -> PreprocessReport:
    ...
    for rec in rows:
        ...
        if replay_path is None or not replay_path.exists():
            continue
        frames = _iter_episode_frames(replay_path, slots, featurize_fn)
        if not frames:
            continue
