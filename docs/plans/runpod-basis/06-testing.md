...
1. **Unit tests**: `bot/src/runpod_io/` の各モジュールを SDK / subprocess / GraphQL レベルで mock し、純粋 Python ロジック (offer 解析、テンプレ置換、run.json schema 拡張、cost 集計、volume CRUD) を完全カバー。`bot/pipeline/imitation/case{1,3,4}/training/train.py` の RunPod env 検出は CPU で deterministic に test。
2. **Integration tests (lite)**: `dev/test-bot` パイプラインに組み込み、`bot/src/runpod_io/` の cli → offers → instance → volumes 結合を `unittest.mock.patch` で stub しつつ end-to-end 実行。実 RunPod は呼ばない。
...
カバレッジ目標は **`bot/src/runpod_io/` で 80%+、`bot/pipeline/imitation/case{1,3,4}/training/train.py` の既存カバレッジを下げない**。

## Unit Tests

### Backend (pytest)

- **`bot/tests/src/vast/test_run_meta.py`** (Step 1)
  - 既存テスト全 pass。
...
- **`bot/tests/pipeline/imitation/case{1,3,4}/training/test_train_run_dir.py`** (Step 2)
...
- **`bot/tests/src/runpod_io/test_auth.py`** (Step 3)
...
- **`bot/tests/src/runpod_io/test_run_meta.py`** (Step 4)
...
- **`bot/tests/src/runpod_io/test_offers.py`** (Step 5)
...
- **`bot/tests/src/runpod_io/test_instance.py`** (Step 6)
...
- **`bot/tests/src/runpod_io/test_onstart_template.py`** (Step 6)
  - `bash -n bot/src/runpod_io/onstart.sh.tmpl` の syntax check (subprocess)。
  - placeholder を valid 値で置換した後の bash -n も pass。

- **`bot/tests/src/runpod_io/test_volumes.py`** (Step 7)
...
- **`bot/tests/src/runpod_io/test_cost.py`** (Step 8)
...
- **`bot/tests/src/runpod_io/test_cli.py`** (Step 9-13)
...
- `dvc repro` を伴う slow integration test は **slow marker** で skip 可能化。`dev/test-bot` のデフォルトでは実行しない (vast 同等)。
...
- **mock の参照 dict**:
  - `bot/tests/src/runpod_io/fixtures/sample_gpus.json`: `runpod.get_gpus()` の固定応答。
  - `bot/tests/src/runpod_io/fixtures/sample_gpu_detail.json`: `runpod.get_gpu(id)` の固定応答 (price 情報含む)。
  - `bot/tests/src/runpod_io/fixtures/sample_create_pod_response.json`: `runpod.create_pod()` の固定応答。
- **run.json fixtures**:
  - `bot/tests/src/runpod_io/fixtures/runs/<dummy_runpod_run>/run.json` (runpod_offer_snapshot あり)。
  - `bot/tests/src/runpod_io/fixtures/runs/<dummy_vast_run>/run.json` (vast_offer_snapshot あり、runpod 集計から除外されることの確認用)。
...
- **`bot/pipeline/imitation/case{1,3,4}/training/train.py`**: 既存カバレッジ維持 (regression 防止)
- **`bot/src/vast/run_meta.py`**: 新フィールド追加分のテスト追加で 100% (default なので分岐ゼロ)
- **`bot/src/runpod_io/onstart.sh.tmpl`**: bash -n syntax check のみ (実行カバレッジは E2E で確認)

## Continuous Integration

- 既存 `dev/test-bot` (`format → lint → type → pytest`) に **新規 `runpod_io/` パッケージのテストを追加**。
...
- `bot/tests/src/vast/` 全テスト pass (Step 1 で `RunMetadata` 拡張時に特に注意)。
- vast 既存 run.json fixtures の読み込みが default 値で埋まり、既存ロジックが動くこと。
- `dev/test-bot` で format/lint/type/pytest が両パッケージ含めグリーン。
