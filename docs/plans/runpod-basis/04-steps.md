...
実装は **vast.run_meta の拡張 → train.py 改修 → runpod_io パッケージ → onstart テンプレ → CLI 4 サブ + volume サブ → e2e + ドキュメント** の順で進める。Step 内の **並列可能** マークは独立着手可能なペアを示す。各 Step で unit test を同時にコミットし、`dev/test-bot` をグリーンに維持する。
...
### Overview
`bot/src/vast/run_meta.RunMetadata` に optional フィールド (`runpod_pod_id`, `runpod_offer_snapshot`) を追加。`schema_version=1` を維持し、既存 vast run.json の読み書きに影響を与えない。

### Work Items
- [ ] `bot/src/vast/run_meta.py` の `RunMetadata` dataclass に `runpod_pod_id: str | None = None` と `runpod_offer_snapshot: dict[str, Any] | None = None` を追加 (default で後方互換)。
- [ ] 既存 `bot/tests/src/vast/test_run_meta.py` に新 field の round-trip テスト追加 (vast 既存テストはそのまま pass)。
...
- `bot/pipeline/imitation/case3/training/train.py`
- `bot/pipeline/imitation/case4/training/train.py`
...
## Step 3: bot/src/runpod_io/auth.py + tests (並列可能 with Step 2)
...
### Work Items
- [ ] `bot/src/runpod_io/__init__.py` を作成、docstring に SDK alias 規約 (`import runpod as runpod_sdk`) を明記。
- [ ] `bot/src/runpod_io/auth.py`:
  ...
  - `load_runpod_api_key(*, env_path: Path | None = None) -> str` 実装 (`bot/.env` → env fallback → actionable error)。
- [ ] `bot/.env.example` に `RUNPOD_API_KEY=your-runpod-api-key` 行を追加。
- [ ] `bot/pyproject.toml` の `dependencies` に `runpod>=1.7.0` 追加。
- [ ] `bot/pyproject.toml` の `[tool.hatch.build.targets.wheel] packages` (or `[tool.setuptools.packages.find]` 等) に `src/runpod_io` 追加。
- [ ] `bot/tests/src/runpod_io/__init__.py` 作成。
- [ ] `bot/tests/src/runpod_io/test_auth.py`:
...
### Target Files
- `bot/src/runpod_io/__init__.py`
- `bot/src/runpod_io/auth.py`
- `bot/.env.example`
- `bot/pyproject.toml`
- `bot/tests/src/runpod_io/__init__.py`
- `bot/tests/src/runpod_io/test_auth.py`

### Acceptance Criteria
- `cd bot && uv sync` が `runpod` をインストール。
- `python -c "import runpod_io; import runpod"` 両方成功。
- pytest pass。

---

## Step 4: bot/src/runpod_io/run_meta.py + tests
...
- [ ] `bot/tests/src/runpod_io/test_run_meta.py`:
...
## Step 5: bot/src/runpod_io/offers.py + tests (並列可能 with Step 6)
...
- [ ] `bot/tests/src/runpod_io/test_offers.py`:
...
## Step 6: bot/src/runpod_io/instance.py + onstart.sh.tmpl + tests (並列可能 with Step 5)
...
- [ ] `bot/tests/src/runpod_io/test_instance.py`:
...
- [ ] `bot/tests/src/runpod_io/test_onstart_template.py`:
  - `bash -n bot/src/runpod_io/onstart.sh.tmpl` でテンプレ自体が syntax error なし (subprocess)。
...
## Step 7: bot/src/runpod_io/volumes.py + tests
...
- [ ] `bot/tests/src/runpod_io/test_volumes.py`:
...
## Step 8: bot/src/runpod_io/cost.py + tests
...
- [ ] `bot/tests/src/runpod_io/test_cost.py`:
...
## Step 9: bot/src/runpod_io/cli.py (stub) + __main__.py + dev/runpod thin wrapper

**Target**: bot, dev tooling
...
### Work Items
- [ ] `bot/src/runpod_io/__main__.py`: `from runpod_io.cli import app; app()`。
- [ ] `bot/src/runpod_io/cli.py`:
...
- [ ] `dev/runpod` (bash thin wrapper):
  ...
  ```
- [ ] `chmod +x dev/runpod`。
- [ ] `bot/tests/src/runpod_io/test_cli.py`: `CliRunner` で `python -m runpod_io --help` が 5 サブコマンド (train/pull/promote/cost-report/volume) を表示することを smoke test。

### Target Files
- `bot/src/runpod_io/__main__.py`
- `bot/src/runpod_io/cli.py`
- `dev/runpod`
- `bot/tests/src/runpod_io/test_cli.py`

### Acceptance Criteria
- `cd bot && uv run python -m runpod_io --help` が 5 サブコマンド表示。
...
- [ ] `bot/tests/src/runpod_io/test_cli.py` に `test_train_*` を追加:
...
### Overview
本物の RunPod pod 起動はせずに、CLI の wiring と `dev/test-bot` グリーンを保つ。実環境 e2e はユーザ判断で 1 度だけ手動実行。

### Work Items
- [ ] `dev/test-bot` を実行してグリーン確認 (format / lint / type / pytest)。
...
- [ ] `bot/pipeline/imitation/case1/README.md` (ある場合) に「両基盤対応」のリファレンス追加。
...
- `bot/pipeline/imitation/case1/README.md` (任意)
...
### Work Items
1. `bot/.env` に `RUNPOD_API_KEY` 設定。
2. RunPod Web UI で network volume を 1 個作成 (DC: US-KS-2, size: 15GB, name: `orbit_wars`)。`bot/.env` に `RUNPOD_NETWORK_VOLUME_ID=<id>` を追加 (任意 step、もしくは `dev/runpod volume create` で実装後)。
