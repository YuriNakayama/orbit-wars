# imitation/case8 — Feature Engineering 拡張 (予測距離 / history / ship 発射)

> 作成日: 2026-05-04
> 関連:
> - `bot/pipeline/imitation/case5/policy/featurizer.py` — 起点 (PLANET_FEAT_DIM=17, GLOBAL_FEAT_DIM=6, ship-prediction 6 列追加済み)
> - `bot/pipeline/imitation/case5/policy/timeline.py` — `simulate_planet_timeline` (case5 内コピー、case8 でも踏襲)
...
>
> スコープ: imitation/case8 を新設 (case5 がベース、case6 は他ブランチで使用中) し、case5 featurizer に **予測距離 / history / 敵 ship 発射 / 自軍 ship 発射** の 4 カテゴリを追加する。
...
評価メトリクス:
- validation: `bot/pipeline/imitation/case8/evaluation/eval_metrics.py` (case3 phase2 形式の per-head metrics)
...
- **AGENT_REGISTRY**: `bot/src/dataset/selfplay/agents.py` に `il_v1` (case1) / `il_v2` (case2) / `il_v3` (case3 phase2) / `il_v4` (case4) / `il_v5` (case5) が登録済み。case8 では `il_v7` を新規登録 (case6 の番号は他ブランチ用に予約)。
- **DVC stages**: `dvc.yaml` に `preprocess_imitation_case<N> / train_imitation_case<N> / eval_imitation_case<N>` が case ごとに並ぶ。case8 用に同形 3 stage を追加。
...
| Path | 変更内容 |
|------|----------|
| `bot/pipeline/imitation/case8/` (新規) | `cp -r case5 case8` で初期化 → 全 import path / module 名を case8 に書き換え |
| `bot/pipeline/imitation/case8/policy/featurizer.py` | PLANET_FEAT_DIM 17 → **24 (+7)**、GLOBAL_FEAT_DIM 6 → **10 (+4)**。新列は下記 "Feature catalogue" 参照 |
| `bot/pipeline/imitation/case8/policy/agent.py` | `HistoryState` を case3 phase2 同等に追加 (`prev_planet_snapshots: deque[maxlen=3]`、`prev_fleets: deque[maxlen=4]`)。featurizer.py が要求する past obs を持ち回す |
| `bot/pipeline/imitation/case8/training/preprocess.py` | replay の per-step 列展開時に `obs_{N-2}` / `obs_{N-3}` / `prev_fleets_{N-1..N-4}` を参照できるよう順次走査。新規列を parquet schema に追加 |
| `bot/pipeline/imitation/case8/training/dataset.py` | input_dim 更新 (planet 24 / global 10) |
| `bot/pipeline/imitation/case8/training/train.py` | input_dim 更新のみ |
| `bot/pipeline/imitation/case8/evaluation/` | `eval_vs_baseline.py` (50 ep self-play) + `eval_metrics.py` (per-head val 精度) |
| `bot/src/dataset/selfplay/agents.py` | `"il_v7": "pipeline.imitation.case8.policy.agent:agent"` を追加 |
| `dvc.yaml` | `preprocess_imitation_case8 / train_imitation_case8 / eval_imitation_case8` stage を追加 |
| `bot/src/runpod_io/cli.py` の `CASE_DEFAULTS` | `case8` entry 追加 |
| `params.yaml` | `imitation.case8.*` ブロック (case5 設定を流用 + input_dim 更新のみ) |
| `tests/pipeline/imitation/case8/` | `test_featurizer_history.py` / `test_featurizer_predicted_distance.py` / `test_history_no_leak.py` (case3 phase2 のリーク回帰防止 — `obs_{N-1}` 経由の delta_ships が action と完全相関しないことを確認) |
...
- case6 (他ブランチで予約) — 触らない
- rulebase 全 case — 触らない (cross-case import 禁止に従い、必要な共通コードは case8 内にコピー)
- Submit shape (case1 が canonical なので影響なし)

## 実装ステップ

1. **case8 ディレクトリの初期化** — `cp -r bot/pipeline/imitation/case5 bot/pipeline/imitation/case8` → `__init__.py` 等の絶対 import path / module docstring を case8 へ書き換え。
...
6. **DVC stage 追加** — `dvc.yaml` に case8 三段 (preprocess / train / eval) を case5 と同形でコピー。`params.yaml` に `imitation.case8.*` ブロックを追加 (input_dim のみ差替え)。
7. **AGENT_REGISTRY 登録** — `il_v7` を `bot/src/dataset/selfplay/agents.py` に追加。
8. **ローカル smoke 検証** — `dev/test-bot` (format / lint / mypy / pytest) → `uv run --directory bot pytest tests/pipeline/imitation/case8 -x` を pass させる。
9. **commit & push** (この plan ではここまで; 以降は `experiment-execution` skill のスコープ)。
10. **(execution skill 側) RunPod Step A** — `dev/runpod train --case case8 --stage preprocess_imitation_case8` でデータ前処理を smoke (≤$0.2)。memory `project_runpod_onstart_pitfalls` の 3 trap (mountpoint-q / dvc pull --allow-missing / cwd-relative config) を pre-run 確認。
11. **(execution skill 側) RunPod Step B** — `dev/runpod train --case case8` で本訓練 (~$1.0)。完了後 `dev/runpod pull <run_id> --case case8`。
12. **(execution skill 側) Stage 1 評価** — `uv run --directory bot python -m pipeline.imitation.case8.evaluation.eval_metrics` で per-head val 精度。case5 比 +0.01 以上ならば Stage 2 へ。
...
# featurizer の単体動作確認
uv run --directory bot python -c "from pipeline.imitation.case8.policy.featurizer import featurize; print(featurize.__doc__)"
```
...
dev/runpod train <commit-sha> --case case8 --stage preprocess_imitation_case8 --watch

# Step B: 本訓練 (~$1.0)
dev/runpod train <commit-sha> --case case8 --watch
dev/runpod pull <run_id> --case case8
```

### 評価

- **Stage 1 (validation)**: case5 baseline (`weights.pt` 既存) と `weights_case8.pt` で `pipeline.imitation.case8.evaluation.eval_metrics` を比較。primary は **from PR-AUC + target macro-F1 + ships macro-F1**。case3 phase2 と同 metric 体系。
- **Stage 2 (self-play sanity, 50 ep)**: `pipeline.imitation.case8.evaluation.eval_vs_baseline --episodes 50 --seed 0` で vs `baseline_v1`。**この 50 ep は採否ではなく、Stage 1 で見えた精度向上が試合に反映されているかの sanity check**。
...
- 既存リポジトリ内: `docs/experiment/imitation/20260503_case5_ship_prediction/plan.md` — case5 の ship-prediction 取り込みと同じ路線で、case8 は cp -r 起点で書き換える。
...
- [ ] case8 ディレクトリが case5 起点で作成され、unit test が pass している
- [ ] `dev/test-bot` が green
- [ ] `dvc.yaml` / `params.yaml` / `agents.py` / `runpod_io.cli` に case8 が登録された
- [ ] commit & push まで完了
