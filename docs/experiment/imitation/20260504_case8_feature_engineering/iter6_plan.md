# imitation/case8 iter6 — K2 leak 除去 + K4 整理 (slim 化)
...
> - `bot/pipeline/imitation/case8/policy/featurizer.py` — iter4 で 61/12 schema、K2 (planet 53-56) と K4 (global 8-11) を保持
>
...
| Path | 変更内容 |
|------|----------|
| `bot/pipeline/imitation/case8/policy/featurizer.py` | PLANET_FEAT_DIM 61→**57**、GLOBAL_FEAT_DIM 12→**8**。K2 (planet 53-56) と K4 (global 8-11) の出力削除、関連 pre-compute 削除 |
| `bot/pipeline/imitation/case8/configs/il_case8.yaml` | `planet_in_dim: 61→57`、`global_in_dim: 12→8` |
| `bot/tests/pipeline/imitation/case8/test_featurizer_dim.py` | `PLANET_FEAT_DIM == 57`、`GLOBAL_FEAT_DIM == 8` に更新 |
| `bot/tests/pipeline/imitation/case8/test_featurizer_iter4.py` | K2/K4 の test を **削除**、K3 frontline (col 53-56 に shift) の test は保持して col 値だけ更新 |
...
2. **`configs/il_case8.yaml` 更新**: `planet_in_dim: 57`, `global_in_dim: 8`
3. **テスト更新**:
...
dev/runpod train <iter6-sha> --case case8 \
...
- [ ] case8 featurizer/configs/tests が iter6 仕様 (PLANET=57, GLOBAL=8) に整理、unit test pass
- [ ] `dev/test-bot` green
- [ ] commit & push
...
- iter5 permutation importance JSON: `data/output/experiment/imitation_case8_iter4_grouped_importance.json`
