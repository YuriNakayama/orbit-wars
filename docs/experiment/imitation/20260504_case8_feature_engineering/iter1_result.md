# imitation/case8 — Feature Engineering 拡張 Result

> 作成日: 2026-05-05
> 関連 plan: `./plan.md`
> commit: `5a89082` (RunPod 修正一連 + mark_progress) → train run `20260505-040940__feature-feature-engineering__5a89082__seed0`
> weights: `bot/pipeline/imitation/case8/policy/weights.pt` (best.pt epoch 9, val_loss=3.5235)
...
case3 phase2 の指標体系で計算 (`bot/pipeline/imitation/case8/evaluation/diagnose_weights.py`)。
**+0.01 以上で採用 → from PR-AUC で +0.136 という大幅改善**で完全突破。

| head | metric | case3 phase2 (ref) | **case8** | Δ | 採否 |
|------|--------|-------------------:|----------:|----:|------|
...
- **from head が最大の伸び**: PR-AUC +0.136 は case3 phase2 から case8 までの新規 11 列
...
- **target head は控えめだが改善**: macro F1 +0.013、top-1 +0.009。template 別 break-down は計測対象外
  (case8 の diagnose_weights は per-template F1 を出力していない、case3 phase2 と差分あり)。
- **ships head はトレードオフ**: acc / MAE は良化したが macro F1 は若干後退。bucket 0/1 (低 ratio) の
  ...
  大躍進だったので、case8 では別 head とのバランスで何かが押し出された。

### Causal leak 回帰防止 (Risk #1) の検証

case8 の `tests/pipeline/imitation/case8/test_featurizer_history.py` は、`obs_{N-2}` 参照の delta_ships
...
- 結局 **case8 が「validation 上は改善、試合上は未確認」** という特殊な状況。
...
1. **300 ep self-play vs baseline_v1** (case8/evaluation/eval_vs_baseline.py -n 300、ローカル ~30 分)
...
case8 を初めて RunPod で full preprocess + train した結果、**6 つの未知 trap** を発見・修正した
...
| 7 | mark_progress 関数が module に存在しない | `5a89082` (`runpod_io.progress.mark_progress` 実装) | 長年の dead import (case3-5 全て該当)、case8 train で初顕在化 |
...
- featurizer: `bot/pipeline/imitation/case8/policy/featurizer.py`
- agent: `bot/pipeline/imitation/case8/policy/agent.py`
- weights: `bot/pipeline/imitation/case8/policy/weights.pt` (982 KB, best @ epoch 9)
- 訓練 metrics: `data/output/models/imitation/case8/runs/20260505-040940__.../metrics.json`
- val metrics: `/tmp/case8_val_metrics.json` (move to `data/output/experiment/imitation_case8_val_metrics.json` if needed)
- 50ep self-play: `/tmp/case8_eval_vs_baseline_50ep_v2.json` (move to `data/output/experiment/imitation_case8_eval_vs_baseline_50ep.json`)
...
4. **DVC + multi-case `dvc.yaml` + RunPod の組み合わせは未踏領域**: 過去の運用は
   `dvc.yaml` の outs 登録なしで `*.parquet.dvc` 戦略だった可能性。**case8 が初の
...
- [ ] case8 の onstart 結果を踏まえ、case5 RunPod training も再開可能 (同じ修正がそのまま使える)
