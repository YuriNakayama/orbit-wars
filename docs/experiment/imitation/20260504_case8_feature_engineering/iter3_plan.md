# imitation/case8 iter3 — Pairwise Top-K + Defense surplus + Sparse mask
...
> - `bot/pipeline/imitation/case8/policy/featurizer.py` — iter2 で 34 列 / 10 → 14 列 に拡張済
>
...
| Path | 変更内容 |
|------|----------|
| `bot/pipeline/imitation/case8/policy/featurizer.py` | PLANET_FEAT_DIM 34 → **約 60-65** (+25-30)、GLOBAL_FEAT_DIM 14 → **14 (変更なし)**。下記 catalogue の列を追加 |
| `bot/pipeline/imitation/case8/configs/il_case8.yaml` | `model.planet_in_dim: 34 → <new>`。他は iter2 と同じ (epochs=15, lr=1e-3, batch=256) |
| `bot/pipeline/imitation/case8/training/preprocess.py` / `dataset.py` | dim 変更のみ (constants 経由で自動追従) |
| `bot/tests/pipeline/imitation/case8/test_featurizer_iter3.py` (新規) | A / D / J 各カテゴリ 1-2 テスト + sparse mask の 0/1 値域 + Pairwise Top-K の K=5 default + Defense surplus の符号 (margin > 0 / < 0) |
...
2. **`configs/il_case8.yaml` 更新**: `planet_in_dim: 63`
...
dev/runpod train <iter3-sha> --case case8 \
...
2. **Permutation feature importance** を val.parquet 上で計算:
   ...
   - 出力: `data/output/experiment/imitation_case8_iter3_feature_importance.json`
...
- [ ] case8 featurizer が iter3 仕様 (PLANET_FEAT_DIM=63) に拡張、unit test pass
- [ ] `dev/test-bot` green
- [ ] commit & push
