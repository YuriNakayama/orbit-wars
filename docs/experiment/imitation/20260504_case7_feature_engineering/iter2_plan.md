# imitation/case7 iter2 — 特徴量追加 (template prefix / fleet trajectory / multi-horizon / production-comet)

> 作成日: 2026-05-05
> 関連:
> - `./iter1_plan.md` — case7 iter1 の plan (predicted dist + history + ship event 11 列追加)
> - `./iter1_result.md` — case7 iter1 の結果 (Stage 1 from PR-AUC +0.136 ✅、Stage 2 0/50 ⚠、採否保留)
> - `bot/pipeline/imitation/case7/policy/featurizer.py` — iter1 で 24 列 / 10 global 列に拡張済み
...
- **case7 iter1 比 +0.01 以上**を全 head で達成
...
評価メトリクス:
- **case7/evaluation/diagnose_weights.py** で per-head val metrics (case3 phase2 と同体系)
...
- dvc.yaml の case7 stage 構造 (deps の hash が変わるので重 rebuild になるのは想定通り)
...
4. **configs/il_case7.yaml 更新**: `planet_in_dim: 42`, `global_in_dim: 14`。
...
6. **ローカル smoke**: `dev/test-bot` (format/lint/mypy/pytest) green 化。`uv --project bot run python -c "from pipeline.imitation.case7.policy.agent import agent; print(agent)"` で import sanity。
7. **commit & push** (この plan は execution skill のスコープ外、`/experiment-execution` で実施)。
8. (execution side) **RunPod Step B 一発**: `dev/runpod train <sha> --case case7 --gpu-name "NVIDIA GeForce RTX 4090" --gpu-name "NVIDIA GeForce RTX 3090" --gpu-name "NVIDIA RTX A6000"`. preprocess は dim 変更で再実行 + train。`dvc.lock` の case7 stage が deps hash 変更で invalidate されるので preprocess 自動 rerun (~7 分) + train (~7 分) の合計 ~25 分予想。
...
# case7 iter2 unit test のみ (新規追加 test を含む)
...
uv run --directory bot pytest tests/pipeline/imitation/case7 -k "no_leak" -x
```
...
dev/runpod train <iter2-commit-sha> --case case7 \
...
- **Stage 1 (validation)**:
  - `uv --project bot run python -m pipeline.imitation.case7.evaluation.diagnose_weights --config bot/pipeline/imitation/case7/configs/il_case7.yaml --weights bot/pipeline/imitation/case7/policy/weights.pt --report data/output/experiment/imitation_case7_iter2_val_metrics.json`
...
- **Ablation (採用後必須)**:
  ...
  - `data/output/experiment/imitation_case7_iter2_ablation_table.md` (or json) に整理、`iter2_result.md` 内に転載
...
- [ ] case7 featurizer/timeline/templates が iter2 仕様 (PLANET_FEAT_DIM=42, GLOBAL_FEAT_DIM=14) に拡張され、unit test が pass
