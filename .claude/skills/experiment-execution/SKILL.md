...
description: >
  ...
  GPU training on RunPod via `dev/runpod train` → report progress periodically →
...
Before spawning the agent, the user should know what they're authorizing. In particular, **GPU training on RunPod costs real money**, so don't sleepwalk into a launch.
...
- **Compute target**: local pytest only, or include a RunPod run? If RunPod, present a brief one-line cost expectation so the user opts in knowingly.
- **Scope of this skill invocation**: full pipeline (plan → impl → test → runpod → result), partial (e.g. plan.md only, or "just write up the result of run xyz"), or "dry-run plan only".

If the user already said `dry-run のプランだけ` or `plan.md だけ書いて`, do not ask about RunPod — they've already scoped it down.
...
If `git status` is dirty and the user wants a RunPod launch, surface that to the user and decide together: commit first, stash, or proceed without RunPod. Do NOT silently launch from a dirty tree.
...
4. **Explicit non-goals** — e.g. "do not run `dev/submit submit` to Kaggle", "do not promote weights via `dev/runpod promote`", "only write `plan.md`, do not implement code". Be specific; the agent is conservative but not psychic.
5. **What to return** — a short report covering: where `plan.md` was written, what was implemented, test status, runpod run id (if launched), ETA, next-step recommendation.
...
- Compute: full pipeline including RunPod GPU training
...
- Do NOT promote weights (dev/runpod promote) without explicit follow-up.

Please execute Phases 2–6 of your workflow and return:
- plan.md path
- pytest result (pass/fail summary)
- runpod run_id, case, ETA
...
- **RunPod status checks while the run is in flight.** When the user asks for an update later, you can either re-spawn `experimenter` with the run id, or run `dev/runpod status <run_id>` / `dev/runpod pull <run_id>` directly and report metrics. Re-spawning is preferable if the user also wants a written `result.md` update.
- **Promotion / submission asks.** If after `result.md` the user wants `dev/runpod promote <run_id>` or a Kaggle submission, treat each as its own approval gate per `.claude/rules/command.md` — these are irreversible and need explicit confirmation immediately before each action.
...
- **RunPod GPU spend is real money.** First launch in a session needs explicit user opt-in. Subsequent launches in the same conversation under the same approved cost envelope do not need re-confirmation.
...
|---|---|
| "imitation/case1 で dropout を 0.3 にして回して" | 1 question (確認: RunPod 含めて full run でよいか) → spawn agent for full pipeline. |
| "case4 の plan.md だけ先に書いて" | Skip RunPod question; spawn agent with `dry-run / plan.md only` non-goal. |
| "さっきの run xxx の結果を docs にまとめて" | Skip Phases 1–2; spawn agent in eval/report-only mode with the run id. |
| "rulebase/case2 を新しく作って harass policy 試して" | Confirm new vs existing case → confirm RunPod → spawn full pipeline. |
| "今 RunPod で回ってる run は終わった?" | No agent needed; run `dev/runpod status <run_id>` (or `dev/runpod pull <run_id>`) directly and report. (If the user also wants result.md, then spawn agent.) |
