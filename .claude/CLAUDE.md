...
`uv run ...` is expected to run under `bot/`. From the repo root, use `dev/*` or `cd bot` first. See [`.claude/rules/command.md`](rules/command.md) for the command catalog (DVC, Vast.ai, Kaggle submission policy).
