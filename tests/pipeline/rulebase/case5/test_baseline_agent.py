"""Integration + snapshot tests for the case5 agent."""
...
from pipeline.rulebase.case5.baseline import agent  # noqa: E402
...
def test_agent_snapshot_first_turn_action() -> None:
    """Phase B equivalence guard: agent is deterministic given a fixed obs."""
    if not SNAPSHOT_OBS.exists() or not SNAPSHOT_ACTION.exists():
        pytest.skip(
            "snapshot files not found — run "
            "`uv run python -m pipeline.rulebase.case5.evaluation"
...
def test_agent_returns_valid_action_shape() -> None:
    """Synthetic obs sanity check: shape and types of returned actions."""
    obs = {
        "player": 0,
        "step": 20,
        "planets": [
