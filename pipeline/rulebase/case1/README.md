...
kaggle kernels pull sigmaborov/lb-897-orbit-wars-2026-reinforce \
  -p pipeline/rulebase/case1/notebook/ -m
```
...
```bash
uv run ruff format --check pipeline/rulebase/case1 tests/pipeline/rulebase/case1
uv run ruff check pipeline/rulebase/case1 tests/pipeline/rulebase/case1
uv run mypy pipeline/rulebase/case1
uv run pytest tests/pipeline/rulebase/case1 -v
# 高速レーン (snapshot を除外)
uv run pytest tests/pipeline/rulebase/case1 -v -m "not slow"
```

カバレッジは本 feature では `--cov=src` のため pipeline/rulebase/case1 は測定対象外。代わりに snapshot 一致性で品質を担保する。
