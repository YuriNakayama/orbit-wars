...
class WorldModel:
    def __init__(
        ...
        recently_captured_ids: set[int] | None = None,
    ) -> None:
        self.player = player
        self.step = step
        ...
        self.recently_captured_ids: set[int] = set(recently_captured_ids or ())
    ...
    def _compute_defense_buffers(
        ...
        for planet in self.my_planets:
            ...
            if not timeline["holds_full"] and timeline["fall_turn"] is not None:
                ...
                prod_ok = planet.production >= REINFORCE_MIN_PRODUCTION
                fresh_ok = (
                    REINFORCE_FRESH_CAPTURE_ENABLED
                    and planet.id in self.recently_captured_ids
                )
                if (
                    REINFORCE_ENABLED
                    and (prod_ok or fresh_ok)
