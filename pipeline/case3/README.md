...
動く PyTorch エージェント。`pipeline/case1` (rule-based) との 1v1 勝率 ≥ 50% を目標とする。
...
uv run python -m pipeline.case3.training.preprocess --config pipeline/case3/configs/il_baseline.yaml

# 2) 学習 (BC)
uv run python -m pipeline.case3.training.train --config pipeline/case3/configs/il_baseline.yaml

# 3) ローカル評価 (vs case1_baseline_v1, 100 戦)
uv run python -m pipeline.case3.evaluation.eval_vs_baseline --episodes 100 --seed 0
```
...
uv run pytest tests/pipeline/case3 -v   # determinism 含む
```

## 設計原則

- **case 間独立**: `pipeline/case3/` は `pipeline/case[012]/` に依存しない / されない。
